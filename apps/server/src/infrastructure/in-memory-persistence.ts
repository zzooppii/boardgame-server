...
import { PandemicGameStateAdapter, type PandemicLifecycle } from "../games/pandemic/compatibility/adapter.js";
...
import { AvalonGameStateAdapter, type AvalonLifecycle } from "../games/avalon/compatibility/adapter.js";
...
import { AvalonSettingsSchema } from "@hangul-rummikub/shared";
...
type RoomGameLifecycleInspection =
  ...
  | Readonly<{gameType:"PANDEMIC";inspection:PandemicLifecycle}>
  ...
  | Readonly<{gameType:"AVALON";inspection:AvalonLifecycle}>
...
function cloneRoomWriteCandidate(
  ...
  switch (candidate.gameType) {
    ...
    case "PANDEMIC": {
      const adapter = new PandemicGameStateAdapter(), game = candidate.game === null ? null : adapter.cloneAndValidate(candidate.game);
      validateRoomGameCoherence(shell.phase, shell.players, game, () => game === null ? null : adapter.inspectLifecycle(game));
      const departedPlayerIds = Object.freeze([...(candidate.departedPlayerIds ?? [])]);
      if (new Set(departedPlayerIds).size !== departedPlayerIds.length || departedPlayerIds.some(id => !shell.players.some(p => p.playerId === id)) || shell.phase === "LOBBY" && departedPlayerIds.length > 0) throw new Error("Invalid departed PANDEMIC roster.");
      return Object.freeze({...shell, gameType:"PANDEMIC", game, departedPlayerIds});
    }
    case "PERCH": {
      const adapter = new PerchGameStateAdapter(), game = candidate.game === null ? null : adapter.cloneAndValidate(candidate.game);
      validateRoomGameCoherence(shell.phase, shell.players, game, () => game === null ? null : adapter.inspectLifecycle(game));
      const departedPlayerIds = Object.freeze([...(candidate.departedPlayerIds ?? [])]);
      if (new Set(departedPlayerIds).size !== departedPlayerIds.length || departedPlayerIds.some(id => !shell.players.some(p => p.playerId === id)) || shell.phase === "LOBBY" && departedPlayerIds.length > 0) throw new Error("Invalid departed PERCH roster.");
      return Object.freeze({...shell, gameType:"PERCH", game, departedPlayerIds});
    }
    case "HARMONIES": {
      const adapter = new HarmoniesGameStateAdapter(), game = candidate.game === null ? null : adapter.cloneAndValidate(candidate.game);
      validateRoomGameCoherence(shell.phase, shell.players, game, () => game === null ? null : adapter.inspectLifecycle(game));
      const departedPlayerIds = Object.freeze([...(candidate.departedPlayerIds ?? [])]);
    ...
    case "ARNAK": {
      const adapter = new ArnakGameStateAdapter(), game = candidate.game === null ? null : adapter.cloneAndValidate(candidate.game);
      validateRoomGameCoherence(shell.phase, shell.players, game, () => game === null ? null : adapter.inspectLifecycle(game));
      const departedPlayerIds = Object.freeze([...(candidate.departedPlayerIds ?? [])]);
    ...
    case "AVALON": {
      const adapter = new AvalonGameStateAdapter(), game = candidate.game === null ? null : adapter.cloneAndValidate(candidate.game);
      validateRoomGameCoherence(shell.phase, shell.players, game, () => game === null ? null : adapter.inspectLifecycle(game));
      const departedPlayerIds = Object.freeze([...(candidate.departedPlayerIds ?? [])]);
      if (new Set(departedPlayerIds).size !== departedPlayerIds.length || departedPlayerIds.some(id => !shell.players.some(p => p.playerId === id)) || shell.phase === "LOBBY" && departedPlayerIds.length > 0) throw new Error("Invalid departed AVALON roster.");
      const settings = v.parse(AvalonSettingsSchema, candidate.settings ?? { roleSet: "INTRIGUE" });
      if (game && JSON.stringify(settings) !== JSON.stringify(game.state.settings)) throw new Error("AVALON settings changed during game.");
      return Object.freeze({...shell, gameType:"AVALON", game, departedPlayerIds, settings});
    }
    case "LIAR_GAME": {
...
function inspectRoomGame(
  ...
  switch (room.gameType) {
    ...
    case "PANDEMIC": return {gameType:"PANDEMIC",inspection:new PandemicGameStateAdapter().inspectLifecycle(room.game)};
    ...
    case "AVALON": return {gameType:"AVALON",inspection:new AvalonGameStateAdapter().inspectLifecycle(room.game)};
