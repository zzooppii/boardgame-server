...
export function transitionTrain(room: TrainRoomRecord, state: TrainState, at: ServerTime): Omit<TrainRoomRecord, 'storageRevision'> {
    if (!room.game)
        throw new Error('Missing Train game.');
...
export class TrainService {
  ...
  async timeout(input:ScheduledTurnDeadline):Promise<{status:'NO_OP'|'APPLIED'|'FAILED'}> {
    ...
    async start(input: StartGameInput): Promise<GameStartResult> {
        ...
        try {
            const result = await d.roomMutationExecutor.run(input.roomId, async (): Promise<GameStartResult> => {
                ...
                if (room?.gameType !== 'TRAIN' || room.departedPlayerIds?.includes(input.actorPlayerId) || !room.players.some(p => p.playerId === input.actorPlayerId))
    ...
    async command(input: Readonly<{
        ...
        command: TrainClientCommand;
        receivedAt: ServerTime;
        authorization: {
            isCurrent(): boolean;
        };
    }>) {
        const parsed = v.safeParse(TrainClientCommandSchema, input.command);
        if (!parsed.success)
        ...
        try {
            const result = await d.roomMutationExecutor.run(input.roomId, async () => {
                ...
                if (c.turnId !== s.transitionId)
                    return failure('STALE_GAME_REVISION');
                const applied = applyTrainAction(s, input.actorPlayerId, c.payload, now, d.ids.generateTurnId(), max => d.random.nextInt(max));
                if (!applied.ok)
                    return failure(applied.reason === 'INVALID_ACTION' ? 'RULE_VIOLATION' : applied.reason);
                const committed = await d.roomUnitOfWork.commit({ roomMutation: { kind: 'REPLACE', candidate: transitionTrain(room, applied.state, now), expectedRoomRevision: room.roomRevision, expectedStorageRevision: room.storageRevision }, sessionMutation: { kind: 'NONE' }, idempotency: { scopeKey, requestId: c.requestId, payloadFingerprint, terminalResult: { outcome: 'ACCEPTED' }, createdAt: now } }, { isSatisfied: () => input.authorization.isCurrent() });
