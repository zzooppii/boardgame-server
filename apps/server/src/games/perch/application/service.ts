...
import { PerchClientCommandSchema, GameRevisionSchema, RoomRevisionSchema, ServerTimeSchema, type PerchClientCommand, type ErrorDto, type RoomId, type PlayerId, type ServerTime } from "@hangul-rummikub/shared";
...
import type { PerchRoomRecord } from "../../../model/persistence.js";
import { createPerchGame, applyPerchAction, type PerchState } from "../domain/game.js";
export type PerchDependencies = Readonly<{
...
const failure = (code: ErrorDto['code']) => ({ ok: false as const, error: { code, message: code === 'RULE_VIOLATION' ? '현재 단계, 행동 차례와 선택 가능한 대상을 확인해주세요.' : code === 'NOT_ENOUGH_PLAYERS' ? '퍼치는 2~5명이 플레이합니다.' : '현재 차례와 연결 상태를 확인해주세요.', recoverable: true } });
const Receipt = v.strictObject({ outcome: v.literal('ACCEPTED') });
export function transitionPerch(room: PerchRoomRecord, state: PerchState, at: ServerTime): Omit<PerchRoomRecord, 'storageRevision'> {
    if (!room.game)
        throw new Error('Missing Perch game.');
...
export class PerchService {
    ...
    constructor(readonly deps: PerchDependencies) { }
    ...
    async start(input: StartGameInput): Promise<GameStartResult> {
        ...
        try {
            return await d.roomMutationExecutor.run(input.roomId, async (): Promise<GameStartResult> => {
                ...
                if (room?.gameType !== 'PERCH' || room.departedPlayerIds?.includes(input.actorPlayerId) || !room.players.some(p => p.playerId === input.actorPlayerId))
                ...
                const state = createPerchGame({ generateTileId: () => d.ids.generateTileId(), gameId, playerIds: room.players.map(p => p.playerId), now, turnId, random: d.random });
    ...
    async command(input: Readonly<{
        ...
        command: PerchClientCommand;
        receivedAt: ServerTime;
        authorization: {
            isCurrent(): boolean;
        };
    }>) {
        const parsed = v.safeParse(PerchClientCommandSchema, input.command);
        if (!parsed.success)
        ...
        try {
            const result = await d.roomMutationExecutor.run(input.roomId, async () => {
                ...
                if (c.turnId !== s.transitionId)
                    return failure('STALE_GAME_REVISION');
                const applied = applyPerchAction(s, input.actorPlayerId, c.payload, now, d.ids.generateTurnId(), d.random, room.hostPlayerId, () => d.ids.generateTileId());
                if (!applied.ok)
                    return failure(applied.reason === 'INVALID_ACTION' ? 'RULE_VIOLATION' : applied.reason);
                const committed = await d.roomUnitOfWork.commit({ roomMutation: { kind: 'REPLACE', candidate: transitionPerch(room, applied.state, now), expectedRoomRevision: room.roomRevision, expectedStorageRevision: room.storageRevision }, sessionMutation: { kind: 'NONE' }, idempotency: { scopeKey, requestId: c.requestId, payloadFingerprint, terminalResult: { outcome: 'ACCEPTED' }, createdAt: now } }, { isSatisfied: () => input.authorization.isCurrent() });
