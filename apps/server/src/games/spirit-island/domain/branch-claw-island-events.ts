...
export function islandEventOptions(s:SpiritState,e:SpiritStep,add:Add):boolean {
 ...
 if(e.key==='BCE3_PROTECT') {
  ...
  if(slots(s,e.tags[0]).reduce((n,p)=>n+p.count,0)>=2)add(`${e.tags[0]} · 현신 2개로 보호한다`,()=>prepend(s,{...e,key:'BCE3_PLEDGE',used:[]}));
  ...
  if(e.used.length<2)for(const slot of available.filter(p=>e.used.filter(key=>key===p.key).length<p.count))add(`${label(slot)} 현신 선택 (${e.used.length}/2)`,()=>prepend(s,{...e,used:[...e.used,slot.key]}),slot.land.id);
  if(e.used.length===2)add('현신 2개 희생 확정 · 이 보드 보호',()=>{
  ...
  for(const l of areas(s).filter(l=>l.board===e.tags[0]&&l.adjacent.some(id=>land(s,id).blight>0)))add(`${l.id} · 인접 오염에서 확산`,()=>prepend(s,step('BLIGHT',e.actor,l.id,1)),l.id);
