...
import { step, prepend, countPieces, invaders, health } from './primitives.js';
...
export function branchFearOptions(s:SpiritState,e:SpiritStep,add:Add):boolean {
 ...
 if(key==='departure'||key==='pestilent'&&level>=2||key==='dangerous'&&level===3)add('이번 정령의 선택 생략',()=>undefined);
 return true;
}
