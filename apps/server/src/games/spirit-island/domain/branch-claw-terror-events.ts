import { relicInvaders } from './forgotten-scenario.js';
...
export function exploreHasSource(s:SpiritState,l:SpiritLand):boolean {
 ...
 for(let depth=0;depth<=distance;depth++){
  if(frontier.some(a=>relicInvaders(s,a).some(p=>p.kind==='TOWN'||p.kind==='CITY')&&!(s.flags.includes('quarantine-source')&&a.tokens.disease>0)||depth<distance&&a.coastal))return true;
