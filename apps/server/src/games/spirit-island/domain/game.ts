            ...
            switch (a.kind) {
                ...
                case 'ADVANCE': {
                    ...
                    if (s.stage === 'FEAR') {
                        ...
                        if(s.ravage&&s.flags.includes('event-next-build')){s.flags=s.flags.filter(f=>f!=='event-next-build');s.flags.push('event-converting-build');s.queue.push(...areas.flatMap(l=>[step('SPECIAL',actor,l.id,0,'BUILD_CARD_LAND'),step('CHECK',actor)]),step('SPECIAL',actor,null,0,'BCE8_BUILD_END'),step('SPECIAL',actor,null,0,'STAGE_BUILD'));}
                        else s.queue.push(step('SPECIAL',actor,null,0,'BCE5_NORMAL_START'),...(areas.some(l=>s.flags.includes(`ruin:${l.id}`))?[step('SPECIAL',actor,null,0,'BCM_RAVAGE_ORDER',actor,areas.map(l=>l.id))]:areas.flatMap(l=>[step('SPECIAL',actor,l.id,0,'RAVAGE'),step('CHECK',actor)])),step('SPECIAL',actor,null,0,'BCE5_NORMAL_END'),step('SPECIAL',actor,null,0,'STAGE_BUILD'));
                    }
