...
export function parseSpiritState(value: unknown): SpiritState {
    ...
    const boards = s.players.map(p => p.board);
    requireRule(new Set(boards).size === s.players.length);
    requireRule(new Set(s.destroyedBoards).size===s.destroyedBoards.length&&s.destroyedBoards.every(b=>boards.includes(b)));
    const links = spiritBoardLinks(boards), landIds = new Set(s.lands.map(l => l.id));
    ...
    for (const l of s.lands) {
        requireRule(boards.includes(l.board) && !s.destroyedBoards.includes(l.board));
...
export function createSpiritGame(input: {
    ...
    const boards = input.shuffle([...SPIRIT_BOARDS]).slice(0, input.playerIds.length);
    const links = spiritBoardLinks(boards);
            ...
            switch (a.kind) {
                ...
                case 'ADVANCE': {
                    ...
                    if (s.stage === 'FEAR') {
                        ...
                        s.queue.push(step('SPECIAL',actor,null,0,'RAVAGE_CARDS'));
                    }
