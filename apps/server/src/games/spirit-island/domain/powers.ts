import { branchMajorPower } from './branch-claw-major.js';
...
/** An unlocked card meets thresholds without granting elements to other powers. */
export function powerSteps(s: SpiritState, actor: PlayerId, key: string, landId: string | null, target: PlayerId, level: number): SpiritStep[] {
    const unlocked=s.flags.includes(`unlocked:${actor}:${key}`);
    if(unlocked)s.flags.push(`threshold-active:${actor}`);
    try { return buildPowerSteps(s,actor,key,landId,target,unlocked?1:level); }
    finally { if(unlocked)s.flags=s.flags.filter(f=>f!==`threshold-active:${actor}`); }
}
/** Build a serializable sequence. Choices and post-move conditions are resolved against the current candidate. */
function buildPowerSteps(s: SpiritState, actor: PlayerId, key: string, landId: string | null, target: PlayerId, level: number): SpiritStep[] {
    const major=branchMajorPower(s,actor,key,landId,target,level);if(major!==null)return major;
