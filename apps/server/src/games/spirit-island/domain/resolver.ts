...
export function automatic(s: SpiritState, e: SpiritStep): boolean {
    if(contactEventAutomatic(s,e)||terrorEventAutomatic(s,e)||industryEventAutomatic(s,e)||settlementEventAutomatic(s,e)||islandEventAutomatic(s,e)||stageEventAutomatic(s,e)||branchEventAutomatic(s,e)||branchMajorAutomatic(s,e)||branchMinorAutomatic(s,e)||branchClawAutomatic(s,e))return true;
    const l = e.land ? land(s, e.land) : null, owner = e.target ?? e.actor, p = player(s, owner);
    if (e.kind === 'CHECK') {
    ...
    switch (e.key) {
        ...
        case 'NEW_ROUND':
            ...
            for (const area of s.lands) {
                ...
                area.vitality = false;
                area.dahanHealth = 0;area.eventHealthLoss=false;area.eventHealthBonus=null;
            }
            s.flags = s.flags.filter(f=>f==='immigration-ended'||f==='event-next-city'||f==='event-next-town'||f==='event-next-city-extra');
