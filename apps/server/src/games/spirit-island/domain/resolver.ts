...
export function choiceOptions(s: SpiritState): SpiritOption[] {
    ...
    if (terrorEventOptions(s,e,add)||industryEventOptions(s,e,add)||settlementEventOptions(s,e,add)||islandEventOptions(s,e,add)||stageEventOptions(s,e,add)||branchEventOptions(s,e,add)||branchMajorOptions(s,e,add)||branchMinorOptions(s,e,add)||branchClawOptions(s,e,add)||tokenOptions(s,e,add)) return list;
...
export function automatic(s: SpiritState, e: SpiritStep): boolean {
    if(terrorEventAutomatic(s,e)||industryEventAutomatic(s,e)||settlementEventAutomatic(s,e)||islandEventAutomatic(s,e)||stageEventAutomatic(s,e)||branchEventAutomatic(s,e)||branchMajorAutomatic(s,e)||branchMinorAutomatic(s,e)||branchClawAutomatic(s,e))return true;
    const l = e.land ? land(s, e.land) : null, owner = e.target ?? e.actor, p = player(s, owner);
    if (e.kind === 'CHECK') {
    ...
    switch (e.key) {
        ...
        case 'NEW_ROUND':
            ...
            s.flags = s.flags.filter(f=>f==='immigration-ended'||f==='event-next-city'||f==='event-next-town');
