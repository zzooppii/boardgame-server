...
export function transitionDuel(room: DuelRoomRecord, state: DuelState, at: ServerTime): Omit<DuelRoomRecord, 'storageRevision'> {
    if (!room.game)
        throw new Error('Missing Duel game.');
...
export class DuelService {
  ...
  async timeout(input:ScheduledTurnDeadline):Promise<{status:'NO_OP'|'APPLIED'|'FAILED'}> {
    ...
    async start(input: StartGameInput): Promise<GameStartResult> {
        ...
        try {
            const result = await d.roomMutationExecutor.run(input.roomId, async (): Promise<GameStartResult> => {
                ...
                if (room?.gameType !== 'SEVEN_WONDERS_DUEL' || room.departedPlayerIds?.includes(input.actorPlayerId) || !room.players.some(p => p.playerId === input.actorPlayerId))
                ...
                const state = createDuelGame({ generateTileId: () => d.ids.generateTileId(), gameId, playerIds: room.players.map(p => p.playerId), now, turnId, random: d.random, settings: room.settings ?? DUEL_DEFAULT_SETTINGS });
    ...
    async command(input: Readonly<{
        ...
        command: DuelClientCommand;
        receivedAt: ServerTime;
        authorization: {
            isCurrent(): boolean;
        };
    }>) {
        const parsed = v.safeParse(DuelClientCommandSchema, input.command);
        if (!parsed.success)
        ...
        try {
            const result = await d.roomMutationExecutor.run(input.roomId, async () => {
                ...
                if (c.turnId !== s.transitionId)
                    return failure('STALE_GAME_REVISION');
                const applied = applyDuelAction(s, input.actorPlayerId, c.payload, now, d.ids.generateTurnId(), d.random);
                if (!applied.ok)
                    return failure(applied.reason === 'INVALID_ACTION' ? 'RULE_VIOLATION' : applied.reason);
                const committed = await d.roomUnitOfWork.commit({ roomMutation: { kind: 'REPLACE', candidate: transitionDuel(room, applied.state, now), expectedRoomRevision: room.roomRevision, expectedStorageRevision: room.storageRevision }, sessionMutation: { kind: 'NONE' }, idempotency: { scopeKey, requestId: c.requestId, payloadFingerprint, terminalResult: { outcome: 'ACCEPTED' }, createdAt: now } }, { isSatisfied: () => input.authorization.isCurrent() });
                if (committed.status !== 'COMMITTED')
                    return failure('STALE_GAME_REVISION');
                changed = true;
