...
import type { DuelDependencies } from "./service.js";
/** Process-local presence history, like the connection registry. Never owns game cards or player actions. */
export class DuelHostSuccession {
    readonly offline = new Map<RoomId, Map<PlayerId, ServerTime>>();
    private timer: ReturnType<typeof setInterval> | undefined;
    constructor(private readonly deps: DuelDependencies, private readonly notify: (roomId: RoomId) => Promise<void>) { }
    disconnected(roomId: RoomId, playerId: PlayerId, at: ServerTime): void {
        ...
        if (!players.has(playerId))
            players.set(playerId, at);
        this.offline.set(roomId, players);
    }
    resumed(roomId: RoomId, playerId: PlayerId): void {
        this.offline.get(roomId)?.delete(playerId);
        void this.evaluate(roomId).catch(() => console.error("SEVEN_WONDERS_DUEL Host policy reconciliation failed."));
    }
    start(): void {
        if (this.timer)
            return;
        this.timer = setInterval(() => {
            for (const roomId of this.offline.keys())
                void this.evaluate(roomId).catch(() => console.error("SEVEN_WONDERS_DUEL Host policy retry failed."));
        }, 1000);
        this.timer.unref();
    }
    stop(): void {
    ...
    async evaluate(roomId: RoomId): Promise<boolean> {
        ...
        const transferred = await d.roomMutationExecutor.run(roomId, async () => {
            ...
            if (!room || room.gameType !== "SEVEN_WONDERS_DUEL") {
            ...
            const result = await d.roomUnitOfWork.commit({
                ...
                idempotency: {
                    ...
                    requestId: parse(RequestIdSchema, `host:${room.roomRevision}:${since}`),
