...
import type { AvalonDependencies } from "./service.js";

/** Process-local presence history, like the connection registry. Never owns secret words, roles or ballots. */
export class AvalonHostSuccession {
  readonly offline = new Map<RoomId, Map<PlayerId, ServerTime>>();
  private timer: ReturnType<typeof setInterval> | undefined;
  constructor(private readonly deps: AvalonDependencies, private readonly notify: (roomId: RoomId) => Promise<void>) {}
  ...
  resumed(roomId: RoomId, playerId: PlayerId): void {
    ...
    void this.evaluate(roomId).catch(() => console.error("AVALON Host policy reconciliation failed."));
  }
  start(): void {
    if (this.timer) return;
    this.timer = setInterval(() => {
      for (const roomId of this.offline.keys()) void this.evaluate(roomId).catch(() => console.error("AVALON Host policy retry failed."));
    }, 1000);
    this.timer.unref();
  }
  ...
  async evaluate(roomId: RoomId): Promise<boolean> {
    ...
    const transferred = await d.roomMutationExecutor.run(roomId, async () => {
      ...
      if (!room || room.gameType !== "AVALON") { this.offline.delete(roomId); return false; }
      ...
      const next = [...room.players].sort((a, b) => a.joinOrder - b.joinOrder).find(p => p.playerId !== host &&
        !room.departedPlayerIds?.includes(p.playerId) && lease.connectionStatusByPlayerId.get(p.playerId) === "CONNECTED");
      if (!next) return false;
      const now = d.clock.now();
      const result = await d.roomUnitOfWork.commit({ roomMutation: { kind: "REPLACE",
        candidate: { ...room, hostPlayerId: next.playerId, roomRevision: parse(RoomRevisionSchema, room.roomRevision + 1), updatedAt: now },
        expectedRoomRevision: room.roomRevision, expectedStorageRevision: room.storageRevision }, sessionMutation: { kind: "NONE" },
        idempotency: { scopeKey: `avalon-host:${roomId}`, requestId: parse(RequestIdSchema, `host:${room.roomRevision}:${since}`),
