...
import { PandemicClientCommandSchema, GameRevisionSchema, RoomRevisionSchema, ServerTimeSchema, type PandemicClientCommand, type ErrorDto, type RoomId, type PlayerId, type ServerTime } from "@hangul-rummikub/shared";
...
import type { PandemicRoomRecord } from "../../../model/persistence.js";
import { createPandemicGame, applyPandemicAction, makePandemicCards, type PandemicState } from "../domain/game.js";

export type PandemicDependencies = Readonly<{ roomRepository:RoomRepository; roomUnitOfWork:RoomUnitOfWork; idempotencyRepository:IdempotencyRepository; roomMutationExecutor:RoomMutationSerialExecutor; presence:RoomPresencePolicyReader; clock:Clock; ids:IdGenerator; random:RandomSource }>;
const failure=(code:ErrorDto['code'])=>({ok:false as const,error:{code,message:code==='RULE_VIOLATION'?'현재 단계, 행동 차례와 카드 비용을 확인해주세요.':code==='NOT_ENOUGH_PLAYERS'?'팬데믹는 2~4명이 플레이합니다.':'현재 차례와 연결 상태를 확인해주세요.',recoverable:true}});
const Receipt=v.strictObject({outcome:v.literal('ACCEPTED')});
export function transitionPandemic(room:PandemicRoomRecord,state:PandemicState,at:ServerTime):Omit<PandemicRoomRecord,'storageRevision'> {
  if(!room.game)throw new Error('Missing Pandemic game.');
...
export class PandemicService {
  ...
  constructor(readonly deps:PandemicDependencies) {}
  ...
  async start(input:StartGameInput):Promise<GameStartResult> {
    ...
    try {return await d.roomMutationExecutor.run(input.roomId,async ():Promise<GameStartResult>=>{
      ...
      if(room?.gameType!=='PANDEMIC'||room.departedPlayerIds?.includes(input.actorPlayerId)||!room.players.some(p=>p.playerId===input.actorPlayerId))return failure('INVALID_PHASE');
      ...
      const state=createPandemicGame({cards:makePandemicCards(()=>d.ids.generateTileId()),gameId,playerIds:room.players.map(p=>p.playerId),now,turnId,random:d.random});
  ...
  async command(input:Readonly<{roomId:RoomId;actorPlayerId:PlayerId;command:PandemicClientCommand;receivedAt:ServerTime;authorization:{isCurrent():boolean}}>) {
    const parsed=v.safeParse(PandemicClientCommandSchema,input.command);if(!parsed.success)return failure('INVALID_PAYLOAD');
    ...
    try {
      const result=await d.roomMutationExecutor.run(input.roomId,async()=>{
        ...
        if(!room.game||room.game.gameId!==c.gameId||room.game.state.revision!==c.expectedGameRevision)return failure('STALE_GAME_REVISION');
