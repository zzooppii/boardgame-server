...
import { marsOffers, marsCardReason, marsCurrentCardCost } from '../domain/game.js';
export function projectMars(game: MarsStoredGame | null, viewer: PlayerId): MarsProjection | null {
    ...
    const payment = s.activePlayerId === viewer && s.payment ? { label: paymentCard ? marsCard(paymentCard.definitionId).name : s.payment.source, cost: s.payment.cost, microbes:s.payment.cardId&&self.hand.some(c=>c.tileId===s.payment?.cardId&&marsCard(c.definitionId).tags.includes('plant'))?(self.played.find(c=>c.definitionId==='Psychrophiles')?.resources??0):0, steel: ['steel', 'both'].includes(s.payment.material), titanium: ['titanium', 'both'].includes(s.payment.material), heat: self.corporationId === 'Helion', ...marsMetalValues(self), cardId: s.payment.cardId, cancelable: s.payment.mode === 'card' } : null;
    const base = { prelude:s.prelude??false, corporateEra: s.corporateEra ?? false, gameType: 'TERRAFORMING_MARS', gameId: s.gameId, gameRevision: s.revision, rulesVersion: s.rulesVersion, generation: s.generation, stage: s.stage, activePlayerId: s.activePlayerId, startingPlayerId: s.startingPlayerId, actionsTaken: s.actionsTaken, oxygen: s.oxygen, temperature: s.temperature, oceans: s.oceans, tiles: s.tiles, landClaims: s.landClaims, milestones: s.milestones, awards: s.awards, history: s.history, playerStates, deckCount: s.deck.length, discardCount: s.discard.length, privateState: { playerId: viewer, preludes:self.preludes, nextCardDiscount: self.nextCardDiscount, hand: self.hand, research: self.research, corporations: s.stage === 'SETUP' ? self.corporations : [], offers: marsOffers(s, viewer), payment, cardChoice: s.cardChoice?.ownerId === viewer ? s.cardChoice.view : null, cardStatus: self.hand.map(c => ({ tileId: c.tileId, cost: marsCurrentCardCost(s,self, marsCard(c.definitionId)), reason: marsCardReason(s, self, marsCard(c.definitionId)) })) } };
