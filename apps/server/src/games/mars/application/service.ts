...
import { MarsClientCommandSchema, GameRevisionSchema, RoomRevisionSchema, ServerTimeSchema, type MarsClientCommand, type ErrorDto, type RoomId, type PlayerId, type ServerTime } from "@hangul-rummikub/shared";
...
import type { MarsRoomRecord } from "../../../model/persistence.js";
import { createMarsGame, applyMarsAction, type MarsState } from "../domain/game.js";
export type MarsDependencies = Readonly<{
...
const failure = (code: ErrorDto['code']) => ({ ok: false as const, error: { code, message: code === 'RULE_VIOLATION' ? '현재 단계, 행동 차례와 선택 가능한 대상을 확인해주세요.' : code === 'NOT_ENOUGH_PLAYERS' ? '테라포밍 마스는 2–5명이 플레이합니다.' : '현재 차례와 연결 상태를 확인해주세요.', recoverable: true } });
const Receipt = v.strictObject({ outcome: v.literal('ACCEPTED') });
export function transitionMars(room: MarsRoomRecord, state: MarsState, at: ServerTime): Omit<MarsRoomRecord, 'storageRevision'> {
    if (!room.game)
        throw new Error('Missing Mars game.');
...
export class MarsService {
    ...
    constructor(readonly deps: MarsDependencies) { }
    ...
    async start(input: StartGameInput): Promise<GameStartResult> {
        ...
        try {
            return await d.roomMutationExecutor.run(input.roomId, async (): Promise<GameStartResult> => {
                ...
                if (room?.gameType !== 'TERRAFORMING_MARS' || room.departedPlayerIds?.includes(input.actorPlayerId) || !room.players.some(p => p.playerId === input.actorPlayerId))
    ...
    async command(input: Readonly<{
        ...
        command: MarsClientCommand;
        receivedAt: ServerTime;
        authorization: {
            isCurrent(): boolean;
        };
    }>) {
        const parsed = v.safeParse(MarsClientCommandSchema, input.command);
        if (!parsed.success)
        ...
        try {
            const result = await d.roomMutationExecutor.run(input.roomId, async () => {
                ...
                if (c.turnId !== s.transitionId)
                    return failure('STALE_GAME_REVISION');
                const applied = applyMarsAction(s, input.actorPlayerId, c.payload, now, d.ids.generateTurnId(), d.random);
                if (!applied.ok)
                    return failure(applied.reason === 'INVALID_ACTION' ? 'RULE_VIOLATION' : applied.reason);
                const committed = await d.roomUnitOfWork.commit({ roomMutation: { kind: 'REPLACE', candidate: transitionMars(room, applied.state, now), expectedRoomRevision: room.roomRevision, expectedStorageRevision: room.storageRevision }, sessionMutation: { kind: 'NONE' }, idempotency: { scopeKey, requestId: c.requestId, payloadFingerprint, terminalResult: { outcome: 'ACCEPTED' }, createdAt: now } }, { isSatisfied: () => input.authorization.isCurrent() });
