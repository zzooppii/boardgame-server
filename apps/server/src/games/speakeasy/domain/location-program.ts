...
export function parseLocationProgress(input:unknown):SpeakeasyLocationProgress {
  ...
  for(const a of s.program.rows.flat()) if(a.kind==='AMBUSH'&&new Set(a.defenses.map(d=>d.shipId)).size!==a.defenses.length) throw new Error('Duplicate ship defense catalog.');
  let waiting=false;
