import {projectArkMultiplayerGame} from '../domain/multiplayer.js';
...
export function projectArkNova(game:ArkNovaStoredGame,viewer:PlayerId):ArkNovaProjection {
  const stored=new ArkNovaGameStateAdapter().cloneAndValidate(game),state='mode' in stored.state?projectArkMultiplayerGame(stored.state,viewer):projectArkSoloGame(stored.state,viewer);
