...
import { ArkNovaGameStateAdapter, type ArkNovaStoredGame } from '../compatibility/adapter.js';
...
export function prepareArkNovaCommand(current:ArkNovaStoredGame,actor:PlayerId,input:unknown,
  ...
  if(!deps.authorization.isCurrent()||actor!==current.state.playerId)return failure('UNAUTHENTICATED');
  ...
  try {
    ...
    const result=applyArkSoloCommand(stored.state,actor,command.expectedGameRevision,command.payload,deps.clock.now(),deps.ids.generateTurnId(),deps.random);
