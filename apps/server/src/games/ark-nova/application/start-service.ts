import {createArkMultiplayerGame} from '../domain/multiplayer.js';
...
const failure=(code:ErrorDto['code'])=>({ok:false as const,error:{code,message:code==='NOT_ENOUGH_PLAYERS'?'아크노바는 1–4명이 플레이합니다.':'현재 연결과 방 상태를 확인해주세요.',recoverable:true}});
export class ArkNovaStartService {
  ...
  async start(input:StartGameInput):Promise<GameStartResult> {
    ...
    try {return await d.roomMutationExecutor.run(input.roomId,async ():Promise<GameStartResult>=>{
      ...
      if(room.players.length<1||room.players.length>4)return failure('NOT_ENOUGH_PLAYERS');
      ...
      const state=room.players.length>1?createArkMultiplayerGame({gameId,playerIds:room.players.map(p=>p.playerId),random:d.random,nextCardId:()=>d.ids.generateTileId(),now,transitionId:turnId}):createArkSoloGame({gameId,playerId:input.actorPlayerId,difficulty:'STANDARD',random:d.random,nextCardId:()=>d.ids.generateTileId(),now,transitionId:turnId});
