...
import { ARK_MAP_A, ArkBuildBonusSchema, arkBorder, arkCellKey, arkNeighbours, type ArkBuilding, type ArkBuildBonus, type ArkCard } from '@hangul-rummikub/shared';
...
export function arkUncoveredPlacementBonuses(buildings:readonly ArkBuilding[]) {
  const covered=new Set(buildings.flatMap(b=>b.cells.map(arkCellKey)));
  return ARK_MAP_A.filter(c=>c.bonus!==null&&!covered.has(arkCellKey(c))).map(c=>({...c,bonus:v.parse(ArkBuildBonusSchema.entries.kind,c.bonus)}));
}
/** Effects for one newly placed building; callers resolve these before placing the next building. */
export function arkNewBuildingEffects(before:readonly ArkBuilding[],building:ArkBuilding,played:readonly ArkCard[]=[]):ArkZooEffect[] {
  const effects:ArkZooEffect[]=arkPlacementBonuses(building).map(b=>arkPlacementBonusEffect(b.kind));
  if(played.some(c=>c.key==='221')) {
    const covered=new Set(building.cells.map(arkCellKey));
    for(const cell of ARK_MAP_A)if(cell.bonus!==null&&covered.has(arkCellKey(cell))&&arkBorder(cell))effects.push({kind:'ARCHAEOLOGIST'});
  }
  for(const [key,terrain] of [['241','WATER'],['242','ROCK']] as const) {
    ...
    const amount=building.cells.filter(cell=>arkNeighbours(cell).some(n=>!covered.has(arkCellKey(n))&&ARK_MAP_A.some(c=>c.terrain===terrain&&arkCellKey(c)===arkCellKey(n)))).length;
    if(amount)effects.push({kind:'GAIN',resource:'MONEY',amount});
  }
  const appeal=arkConstructionAppeal([...before,building])-arkConstructionAppeal(before);
