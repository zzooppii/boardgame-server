import {arkIgnoredAnimalConditions,arkSponsorLevel} from '@hangul-rummikub/shared';
...
export function playArkZooCard<T extends ArkCardPlayState>(current:T,scope:ArkCardPlayScope,input:unknown):
  ...
  const ignores=arkIgnoredAnimalConditions(definition,current);
  ...
  const strengthCost=definition.kind==='ANIMAL'?1:arkSponsorLevel(definition,current);
  if (scope.remaining<strengthCost) return {ok:false};
  const displaySlot=held?0:displayIndex+1;
  const paid=definition.kind==='ANIMAL'?arkAnimalPrice(definition,current,displaySlot):(scope.paySponsorLevel?arkSponsorLevel(definition,current):displaySlot);
  ...
  if (definition.kind==='ANIMAL') {
    if (housingId===null) {
      ...
      const housing=occupyArkAnimalHousing(current.buildings,definition,housingId,current.played.some(c=>c.key==='219'),current.mapId);
  ...
  if (Object.hasOwn(ARK_UNIQUE_BUILDINGS,definition.key)) {
    const placed=validateArkUniqueConstruction(current.buildings,definition,current.actions.some(a=>a.kind==='BUILD'&&a.upgraded),current.played.some(c=>c.key==='219'),choice.output.uniquePlacement,current.mapId);
