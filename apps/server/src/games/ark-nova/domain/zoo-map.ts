 ...
 */
export function assertArkZooMap(buildings:readonly ArkBuilding[],played:readonly ArkCard[],buildUpgraded:boolean):void {
  const all=v.parse(v.array(ArkBuildingSchema),buildings),initial=arkInitialBuildings();
  ...
  for(const [index,b] of all.entries()) {
    if(Object.hasOwn(ARK_BUILDINGS,b.kind)) {
      ...
      if(index<initial.length) {
        ...
        if(!hasShape||arkPlacementReason(placed,b.kind,b.cells,buildUpgraded,ignoreTerrain)!==null)throw new Error('Invalid Ark stored construction.');
      }
    } else {
      ...
      const fits=b.cells.some(anchor=>[0,1,2,3,4,5].some(rotation=>{
        const result=validateArkUniqueConstruction(placed,definition,buildUpgraded,ignoreTerrain,{anchor,rotation});
