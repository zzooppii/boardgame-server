...
export function projectArkEffectGuide(effect:ArkZooEffect,actions:readonly ArkActionCard[],pool:ArkConservationBonusPool):ArkEffectGuide {
  ...
  return {...(effect.kind==='INTERACTION'?{interaction:{ability:effect.ability,amount:effect.amount,...(effect.track?{track:effect.track}:{})}}:{}),...(effect.kind==='MOVE_TO_SPECIAL'?{specialMove:{buildingId:effect.buildingId,moved:[...effect.moved]}}:{}),resource:effect.kind==='GAIN'?effect.resource:null,amount:'amount' in effect&&typeof effect.amount==='number'?effect.amount:null,actions:choices,
