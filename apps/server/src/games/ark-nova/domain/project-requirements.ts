import { ARK_CONTINENTS, arkProject, type ArkCard, type ArkBuilding } from '@hangul-rummikub/shared';
...
export type ArkProjectContext=Readonly<{played:readonly ArkCard[];partners:readonly string[];universities:readonly string[]}>;
...
export function arkReleaseHousingChoices(buildings:readonly ArkBuilding[],animal:ArkCard,ignoreTerrain=false):string[] {
  ...
  return arkEnclosuresToEmpty(buildings,d,ignoreTerrain);
}
...
export function releaseArkProjectAnimal(s:ArkProjectContext & {buildings:readonly ArkBuilding[];appeal:number},key:string,slot:number,animalId:string,housingId:string|null,ignoreTerrain=false):
  ...
  const choices=arkReleaseHousingChoices(s.buildings,animal,ignoreTerrain);
