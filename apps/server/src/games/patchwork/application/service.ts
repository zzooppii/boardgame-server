...
export function transitionPatchwork(room: PatchworkRoomRecord, state: PatchworkState, at: ServerTime): Omit<PatchworkRoomRecord, 'storageRevision'> {
    if (!room.game)
        throw new Error('Missing Patchwork game.');
...
export class PatchworkService {
    ...
    async start(input: StartGameInput): Promise<GameStartResult> {
        ...
        try {
            return await d.roomMutationExecutor.run(input.roomId, async (): Promise<GameStartResult> => {
                ...
                if (room?.gameType !== 'PATCHWORK' || room.departedPlayerIds?.includes(input.actorPlayerId) || !room.players.some(p => p.playerId === input.actorPlayerId))
    ...
    async command(input: Readonly<{
        ...
        command: PatchworkClientCommand;
        receivedAt: ServerTime;
        authorization: {
            isCurrent(): boolean;
        };
    }>) {
        const parsed = v.safeParse(PatchworkClientCommandSchema, input.command);
        if (!parsed.success)
        ...
        try {
            const result = await d.roomMutationExecutor.run(input.roomId, async () => {
                ...
                if (c.turnId !== s.transitionId)
                    return failure('STALE_GAME_REVISION');
                const applied = applyPatchworkAction(s, input.actorPlayerId, c.payload, now, d.ids.generateTurnId());
                if (!applied.ok)
