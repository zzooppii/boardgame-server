...
export function createPatchworkGame(input:{generateTileId():TileId;gameId:GameId;playerIds:readonly PlayerId[];now:ServerTime;turnId:TurnId;random:RandomSource}):PatchworkState{
...
export function applyPatchworkAction(s:PatchworkState,actor:PlayerId,input:PatchworkAction,now:ServerTime,turnId:TurnId):{ok:true;state:PatchworkState}|{ok:false;reason:'INVALID_ACTION'|'INVALID_PHASE'|'NOT_YOUR_TURN'}{
 ...
 settle(next,actor,now);next.revision=v.parse(GameRevisionSchema,s.revision+1);next.transitionId=turnId;
 next.history.push({id:next.revision,playerId:actor,kind:a.type==='PLACE_LEATHER'?'LEATHER':a.type,patchId,distance,income,bonus:s.bonusOwner===null&&next.bonusOwner===actor});
 return {ok:true,state:parsePatchworkState(next)};
}
export function cancelPatchwork(s:PatchworkState,now:ServerTime):PatchworkState{return parsePatchworkState({...s,revision:s.revision+1,phase:'FINISHED',finishedAt:now,result:{reason:'CANCELLED',winnerPlayerIds:[]}});}
