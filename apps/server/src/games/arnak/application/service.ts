...
import { ArnakClientCommandSchema, GameRevisionSchema, RoomRevisionSchema, ServerTimeSchema, type ArnakClientCommand, type ErrorDto, type RoomId, type PlayerId, type ServerTime } from "@hangul-rummikub/shared";
...
import type { ArnakRoomRecord } from "../../../model/persistence.js";
import { createArnakGame, applyArnakAction, type ArnakState } from "../domain/game.js";
export type ArnakDependencies = Readonly<{
...
const failure = (code: ErrorDto['code']) => ({ ok: false as const, error: { code, message: code === 'RULE_VIOLATION' ? '현재 단계, 행동 차례와 선택 가능한 대상을 확인해주세요.' : code === 'NOT_ENOUGH_PLAYERS' ? '아르낙은 2–4명이 플레이합니다.' : '현재 차례와 연결 상태를 확인해주세요.', recoverable: true } });
const Receipt = v.strictObject({ outcome: v.literal('ACCEPTED') });
export function transitionArnak(room: ArnakRoomRecord, state: ArnakState, at: ServerTime): Omit<ArnakRoomRecord, 'storageRevision'> {
    if (!room.game)
        throw new Error('Missing Arnak game.');
...
export class ArnakService {
    private readonly listeners = new Set<(roomId: RoomId) => void | Promise<void>>();
    constructor(readonly deps: ArnakDependencies) { }
    ...
    async start(input: StartGameInput): Promise<GameStartResult> {
        ...
        try {
            return await d.roomMutationExecutor.run(input.roomId, async (): Promise<GameStartResult> => {
                ...
                if (room?.gameType !== 'ARNAK' || room.departedPlayerIds?.includes(input.actorPlayerId) || !room.players.some(p => p.playerId === input.actorPlayerId))
                ...
                const state = createArnakGame({ generateTileId: () => d.ids.generateTileId(), gameId, playerIds: room.players.map(p => p.playerId), now, turnId, random: d.random });
    ...
    async command(input: Readonly<{
        ...
        command: ArnakClientCommand;
        receivedAt: ServerTime;
        authorization: {
            isCurrent(): boolean;
        };
    }>) {
        const parsed = v.safeParse(ArnakClientCommandSchema, input.command);
        if (!parsed.success)
        ...
        try {
            const result = await d.roomMutationExecutor.run(input.roomId, async () => {
                ...
                if (c.turnId !== s.transitionId)
                    return failure('STALE_GAME_REVISION');
                const applied = applyArnakAction(s, input.actorPlayerId, c.payload, now, d.ids.generateTurnId(), d.random);
                if (!applied.ok)
                    return failure(applied.reason === 'INVALID_ACTION' ? 'RULE_VIOLATION' : applied.reason);
                const committed = await d.roomUnitOfWork.commit({ roomMutation: { kind: 'REPLACE', candidate: transitionArnak(room, applied.state, now), expectedRoomRevision: room.roomRevision, expectedStorageRevision: room.storageRevision }, sessionMutation: { kind: 'NONE' }, idempotency: { scopeKey, requestId: c.requestId, payloadFingerprint, terminalResult: { outcome: 'ACCEPTED' }, createdAt: now } }, { isSatisfied: () => input.authorization.isCurrent() });
