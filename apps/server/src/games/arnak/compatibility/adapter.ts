...
import { parseArnakState, type ArnakState } from "../domain/game.js";
export type ArnakStoredGame = Readonly<{
    gameId: GameId;
...
export type ArnakLifecycle = Readonly<{
    lifecycle: 'RUNNING';
    gameId: GameId;
    gameRevision: GameRevision;
    activeTurn: null;
...
export class ArnakGameStateAdapter {
    cloneAndValidate(game: ArnakStoredGame): ArnakStoredGame {
        const state = parseArnakState(game.state), gameId = parse(GameIdSchema, game.gameId), gameRevision = parse(GameRevisionSchema, game.gameRevision), startedAt = parse(ServerTimeSchema, game.startedAt);
        if (state.gameId !== gameId || state.revision !== gameRevision || state.startedAt !== startedAt || state.finishedAt !== game.finishedAt)
            throw new Error('Arnak stored metadata mismatch.');
        return { gameId, gameRevision, startedAt, finishedAt: state.finishedAt, state };
    }
    inspectLifecycle(game: ArnakStoredGame): ArnakLifecycle {
        if (game.state.phase === 'FINISHED') {
            if (game.finishedAt === null)
                throw new Error('Arnak finish time missing.');
