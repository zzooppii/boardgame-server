...
export function transitionHarmonies(room: HarmoniesRoomRecord, state: HarmoniesState, at: ServerTime): Omit<HarmoniesRoomRecord, 'storageRevision'> {
    if (!room.game)
        throw new Error('Missing Harmonies game.');
...
export class HarmoniesService {
    ...
    async start(input: StartGameInput): Promise<GameStartResult> {
        ...
        try {
            return await d.roomMutationExecutor.run(input.roomId, async (): Promise<GameStartResult> => {
                ...
                if (room?.gameType !== 'HARMONIES' || room.departedPlayerIds?.includes(input.actorPlayerId) || !room.players.some(p => p.playerId === input.actorPlayerId))
    ...
    async command(input: Readonly<{
        ...
        command: HarmoniesClientCommand;
        receivedAt: ServerTime;
        authorization: {
            isCurrent(): boolean;
        };
    }>) {
        const parsed = v.safeParse(HarmoniesClientCommandSchema, input.command);
        if (!parsed.success)
