  ...
  socket.on("session:resume", (rawCommand, acknowledge) => {
    ...
    void authenticationExecutor.run(createSocketId(socket.id), async () => {
      ...
      try {
        ...
        runtime.marsHostSuccession?.resumed(result.data.roomId, result.data.playerId);
...
function registerGreatKingdomHandlers(socket: RealtimeSocket, runtime: ApplicationRuntime): void {
  for (const event of ["greatKingdom:act", "greatKingdom:configure"] as const) socket.on(event, (raw: unknown, acknowledge: (ack: StateSyncWireAck) => void) => {
...
import { PandemicClientCommandSchema } from "@hangul-rummikub/shared";
function registerPandemicHandlers(socket: RealtimeSocket, runtime: ApplicationRuntime): void {
  for (const event of ["pandemic:act"] as const) socket.on(event, (raw: unknown, acknowledge: (ack: StateSyncWireAck) => void) => {
    const receivedAt = runtime.clock.now(), command = parseNumberRematch(PandemicClientCommandSchema, raw);
    if (!command.success || command.output.kind !== event) { acknowledgeIfPresent(acknowledge, failureAck(raw, INVALID_PAYLOAD_ERROR, receivedAt)); return; }
    void (async () => {
      const binding = runtime.connectionRegistry.getAuthenticatedBinding(createSocketId(socket.id));
      if (!binding) { acknowledgeIfPresent(acknowledge, failureAck(raw, UNAUTHENTICATED_ERROR, receivedAt)); return; }
      if (!isRoomAdmissionCompatible("PANDEMIC", socketAdmissionCapabilities(socket))) {
        acknowledgeIfPresent(acknowledge, failureAck(raw, {code:"INCOMPATIBLE_GAME_CAPABILITY",message:"PANDEMIC requires V2 capability.",recoverable:false}, receivedAt)); return;
      }
      if (!runtime.pandemicService) { acknowledgeIfPresent(acknowledge, failureAck(raw, INTERNAL_ERROR, receivedAt)); return; }
      const result = await runtime.pandemicService.command({roomId:binding.roomId,actorPlayerId:binding.playerId,command:command.output,receivedAt,
        authorization:{isCurrent:()=>socket.connected && isCurrentBinding(runtime,binding)}});
      if (!result.ok) { acknowledgeIfPresent(acknowledge, failureAck(raw,result.error,receivedAt)); return; }
      const loaded = await loadSnapshotForSocket(runtime,socket,binding.roomId,binding.playerId);
      if (loaded && socket.connected && isCurrentBinding(runtime,binding)) acknowledgeIfPresent(acknowledge,snapshotSuccessAck(command.output.requestId,loaded.metadata,loaded.wireSnapshot));
...
import { PerchClientCommandSchema } from "@hangul-rummikub/shared";
function registerPerchHandlers(socket: RealtimeSocket, runtime: ApplicationRuntime): void {
  for (const event of ["perch:act"] as const) socket.on(event, (raw: unknown, acknowledge: (ack: StateSyncWireAck) => void) => {
    const receivedAt = runtime.clock.now(), command = parseNumberRematch(PerchClientCommandSchema, raw);
    if (!command.success || command.output.kind !== event) { acknowledgeIfPresent(acknowledge, failureAck(raw, INVALID_PAYLOAD_ERROR, receivedAt)); return; }
    void (async () => {
      const binding = runtime.connectionRegistry.getAuthenticatedBinding(createSocketId(socket.id));
      if (!binding) { acknowledgeIfPresent(acknowledge, failureAck(raw, UNAUTHENTICATED_ERROR, receivedAt)); return; }
      if (!isRoomAdmissionCompatible("PERCH", socketAdmissionCapabilities(socket))) {
        acknowledgeIfPresent(acknowledge, failureAck(raw, {code:"INCOMPATIBLE_GAME_CAPABILITY",message:"PERCH requires V2 capability.",recoverable:false}, receivedAt)); return;
      }
      if (!runtime.perchService) { acknowledgeIfPresent(acknowledge, failureAck(raw, INTERNAL_ERROR, receivedAt)); return; }
      const result = await runtime.perchService.command({roomId:binding.roomId,actorPlayerId:binding.playerId,command:command.output,receivedAt,
        authorization:{isCurrent:()=>socket.connected && isCurrentBinding(runtime,binding)}});
      if (!result.ok) { acknowledgeIfPresent(acknowledge, failureAck(raw,result.error,receivedAt)); return; }
      const loaded = await loadSnapshotForSocket(runtime,socket,binding.roomId,binding.playerId);
      if (loaded && socket.connected && isCurrentBinding(runtime,binding)) acknowledgeIfPresent(acknowledge,snapshotSuccessAck(command.output.requestId,loaded.metadata,loaded.wireSnapshot));
...
function registerHarmoniesHandlers(socket: RealtimeSocket, runtime: ApplicationRuntime): void {
  for (const event of ["harmonies:act", "harmonies:configure", "harmonies:draft"] as const) socket.on(event, (raw: unknown, acknowledge: (ack: StateSyncWireAck) => void) => {
    ...
    void (async () => {
      const binding = runtime.connectionRegistry.getAuthenticatedBinding(createSocketId(socket.id));
      if (!binding) { acknowledgeIfPresent(acknowledge, failureAck(raw, UNAUTHENTICATED_ERROR, receivedAt)); return; }
      if (!isRoomAdmissionCompatible("HARMONIES", socketAdmissionCapabilities(socket))) {
        acknowledgeIfPresent(acknowledge, failureAck(raw, {code:"INCOMPATIBLE_GAME_CAPABILITY",message:"HARMONIES requires V2 capability.",recoverable:false}, receivedAt)); return;
      }
      if (!runtime.harmoniesService) { acknowledgeIfPresent(acknowledge, failureAck(raw, INTERNAL_ERROR, receivedAt)); return; }
      const result = await runtime.harmoniesService.command({roomId:binding.roomId,actorPlayerId:binding.playerId,command:command.output,receivedAt,
        authorization:{isCurrent:()=>socket.connected && isCurrentBinding(runtime,binding)}});
      if (!result.ok) { acknowledgeIfPresent(acknowledge, failureAck(raw,result.error,receivedAt)); return; }
      const loaded = await loadSnapshotForSocket(runtime,socket,binding.roomId,binding.playerId);
      if (loaded && socket.connected && isCurrentBinding(runtime,binding)) acknowledgeIfPresent(acknowledge,snapshotSuccessAck(command.output.requestId,loaded.metadata,loaded.wireSnapshot));
...
function registerMarsHandlers(socket: RealtimeSocket, runtime: ApplicationRuntime): void {
  for (const event of ["mars:act", "mars:configure"] as const) socket.on(event, (raw: unknown, acknowledge: (ack: StateSyncWireAck) => void) => {
    ...
    void (async () => {
      const binding = runtime.connectionRegistry.getAuthenticatedBinding(createSocketId(socket.id));
      if (!binding) { acknowledgeIfPresent(acknowledge, failureAck(raw, UNAUTHENTICATED_ERROR, receivedAt)); return; }
      if (!isRoomAdmissionCompatible("TERRAFORMING_MARS", socketAdmissionCapabilities(socket))) {
        acknowledgeIfPresent(acknowledge, failureAck(raw, {code:"INCOMPATIBLE_GAME_CAPABILITY",message:"TERRAFORMING_MARS requires V2 capability.",recoverable:false}, receivedAt)); return;
      }
      if (!runtime.marsService) { acknowledgeIfPresent(acknowledge, failureAck(raw, INTERNAL_ERROR, receivedAt)); return; }
      const result = await runtime.marsService.command({roomId:binding.roomId,actorPlayerId:binding.playerId,command:command.output,receivedAt,
        authorization:{isCurrent:()=>socket.connected && isCurrentBinding(runtime,binding)}});
      if (!result.ok) { acknowledgeIfPresent(acknowledge, failureAck(raw,result.error,receivedAt)); return; }
      const loaded = await loadSnapshotForSocket(runtime,socket,binding.roomId,binding.playerId);
      if (loaded && socket.connected && isCurrentBinding(runtime,binding)) acknowledgeIfPresent(acknowledge,snapshotSuccessAck(command.output.requestId,loaded.metadata,loaded.wireSnapshot));
...
function registerDuelHandlers(socket: RealtimeSocket, runtime: ApplicationRuntime): void {
  for (const event of ["duel:act", "duel:configure"] as const) socket.on(event, (raw: unknown, acknowledge: (ack: StateSyncWireAck) => void) => {
    const receivedAt = runtime.clock.now(), command = parseNumberRematch(DuelClientCommandSchema, raw);
    if (!command.success || command.output.kind !== event) { acknowledgeIfPresent(acknowledge, failureAck(raw, INVALID_PAYLOAD_ERROR, receivedAt)); return; }
    void (async () => {
      const binding = runtime.connectionRegistry.getAuthenticatedBinding(createSocketId(socket.id));
      if (!binding) { acknowledgeIfPresent(acknowledge, failureAck(raw, UNAUTHENTICATED_ERROR, receivedAt)); return; }
      if (!isRoomAdmissionCompatible("SEVEN_WONDERS_DUEL", socketAdmissionCapabilities(socket))) {
        acknowledgeIfPresent(acknowledge, failureAck(raw, {code:"INCOMPATIBLE_GAME_CAPABILITY",message:"SEVEN_WONDERS_DUEL requires V2 capability.",recoverable:false}, receivedAt)); return;
      }
      if (!runtime.duelService) { acknowledgeIfPresent(acknowledge, failureAck(raw, INTERNAL_ERROR, receivedAt)); return; }
      const result = await runtime.duelService.command({roomId:binding.roomId,actorPlayerId:binding.playerId,command:command.output,receivedAt,
        authorization:{isCurrent:()=>socket.connected && isCurrentBinding(runtime,binding)}});
      if (!result.ok) { acknowledgeIfPresent(acknowledge, failureAck(raw,result.error,receivedAt)); return; }
      const loaded = await loadSnapshotForSocket(runtime,socket,binding.roomId,binding.playerId);
      if (loaded && socket.connected && isCurrentBinding(runtime,binding)) acknowledgeIfPresent(acknowledge,snapshotSuccessAck(command.output.requestId,loaded.metadata,loaded.wireSnapshot));
...
function registerDisconnectHandler(
  ...
  socket.on("disconnect", () => {
    ...
    runtime.marsHostSuccession?.disconnected(binding.roomId, binding.playerId, disconnectedAt);
...
export function registerSocketIoHandlers(
  ...
  const unsubscribeMars = runtime.marsService?.subscribe(roomId => fanOutRoomSnapshots(io, runtime, roomId));
  ...
  io.on("connection", (socket) => {
    ...
    registerMarsHandlers(socket, runtime);
