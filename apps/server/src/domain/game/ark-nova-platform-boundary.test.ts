...
function solo(g:ArkNovaStoredGame){assert.ok(!('mode' in g.state));return g.state;}
...
test('Ark stored adapter rejects wrapper drift and returns isolated state and a timer-free lifecycle',()=>{
  ...
  solo(copy).hand.pop();assert.equal(solo(original).hand.length,8);
...
test('Ark wire boundary rejects stale game, revision, transition and untrusted identity fields without changing state',()=>{
  const g=game(),before=structuredClone(g),command=wire(g,{kind:'INITIAL_HAND',keep:solo(g).hand.slice(0,4).map(c=>c.cardId)});
...
test('Ark owner projection has matching metadata and hides deck contents and reserve cards',()=>{
  ...
  assert.equal(view.state.deckCount,solo(g).zooDeck.length);assert.equal(view.state.hand.length,8);
  ...
  view.state.hand.pop();assert.equal(solo(g).hand.length,8);
});
test('Ark wire candidates preserve atomicity from initial choice through 27 turns and finished projection',()=>{
  ...
  assert.equal(solo(g).progress.turnsCompleted,27);assert.equal(projectArkNova(g,owner).phase,'FINISHED');
