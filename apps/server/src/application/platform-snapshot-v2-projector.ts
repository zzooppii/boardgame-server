import { GREAT_KINGDOM_DEFAULT_SETTINGS } from "@hangul-rummikub/shared";
import { CARCASSONNE_DEFAULT_SETTINGS } from "@hangul-rummikub/shared";
import {HARMONIES_DEFAULT_SETTINGS} from "@hangul-rummikub/shared";
...
export class PlatformSnapshotV2Projector {
  ...
  async project(
    ...
    if(input.room.gameType === "TRAIN") return v.parse(PlatformSnapshotV2Schema, {...base,room:{...base.room,phase:input.room.phase,settings:input.room.settings??{mapId:"USA"}},game:projectTrain(input.room.game,input.selfPlayerId)});
    ...
    if(input.room.gameType === "GREAT_KINGDOM") return v.parse(PlatformSnapshotV2Schema, {...base,room:{...base.room,phase:input.room.phase,settings:input.room.settings ?? GREAT_KINGDOM_DEFAULT_SETTINGS},game:projectGreatKingdom(input.room.game,input.selfPlayerId)});
    ...
    if(input.room.gameType === "CARCASSONNE") return v.parse(PlatformSnapshotV2Schema, {...base,room:{...base.room,phase:input.room.phase,settings:input.room.settings??CARCASSONNE_DEFAULT_SETTINGS},game:projectCarcassonne(input.room.game,input.selfPlayerId)});
    ...
    if(input.room.gameType === "PATCHWORK") return v.parse(PlatformSnapshotV2Schema, {...base,room:{...base.room,phase:input.room.phase,settings:input.room.settings??{turnDurationSeconds:60}},game:projectPatchwork(input.room.game,input.selfPlayerId)});
    ...
    if(input.room.gameType === "TERRAFORMING_MARS") return v.parse(PlatformSnapshotV2Schema, {...base,room:{...base.room,settings:input.room.settings ?? {corporateEra:false},phase:input.room.phase},game:projectMars(input.room.game,input.selfPlayerId)});
    ...
    if(input.room.gameType === "AVALON") return v.parse(PlatformSnapshotV2Schema, {...base,room:{...base.room,phase:input.room.phase},game:projectAvalon(input.room.game,input.selfPlayerId)});
