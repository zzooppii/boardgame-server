...
function strifeFear(key:string,level:1|2|3,n=1){const s=branchFearGame(key,level,n);for(const l of s.lands){l.pieces=[];l.tokens.beasts=0;l.tokens.disease=0;}makePiece(s,land(s,'A8'),'CITY');return s;}
for(const key of ['unrest','panic','threaten'])test(`${key} enforces strife target restrictions and mandatory selection`,()=>{
 const s=strifeFear(key,1),a=land(s,'A1');makePiece(s,a,'TOWN');if(key==='panic')a.tokens.beasts=1;if(key==='threaten')makePiece(s,a,'DAHAN');makePiece(s,land(s,'A2'),'EXPLORER');settle(s);assert.deepEqual(choiceOptions(s).map(o=>o.landId),['A1']);const next=eventChoose(s,'A1');assert.equal(choiceOptions(next).length,1);assert.ok(choiceOptions(next)[0]!.label.includes('분쟁 추가'));
});
test('Strife health loss starts after all players add strife and checks pre-existing damage everywhere',()=>{
 let s=strifeFear('unrest',2,2);const a=land(s,'A1'),other=land(s,'A2');makePiece(s,a,'TOWN');const old=makePiece(s,other,'TOWN');old.strife=1;old.damage=1;settle(s);s=eventChoose(s,'A1');s=eventChoose(s,'분쟁 추가');assert.equal(land(s,'A1').strifeHealthLoss,0);assert.equal(s.queue[0]!.target,s.players[1]!.playerId);s=eventChoose(s,'A1');s=drain(s);assert.equal(land(s,'A1').strifeHealthLoss,1);assert.equal(land(s,'A1').pieces[0]!.strife,2);assert.equal(health(land(s,'A1'),land(s,'A1').pieces[0]!),1);assert.equal(countPieces(land(s,'A2'),['TOWN']),0);parseSpiritState(s);
});
test('Strife health penalties stack, respect health bonuses and leave Dahan unchanged',()=>{
 let s=strifeFear('unrest',2);const a=land(s,'A1');makePiece(s,a,'TOWN');makePiece(s,a,'DAHAN');a.invaderHealth=1;settle(s);s=drain(s);let l=land(s,'A1');assert.equal(health(l,l.pieces.find(p=>p.kind==='TOWN')!),2);assert.equal(health(l,l.pieces.find(p=>p.kind==='DAHAN')!),2);s.queue=[step('SPECIAL',s.players[0]!.playerId,null,0,'FEAR_CARD',null,['unrest'])];settle(s);s=drain(s);l=land(s,'A1');assert.equal(l.strifeHealthLoss,2);assert.equal(health(l,l.pieces.find(p=>p.kind==='TOWN')!),1);s.stage='TIME';s=drain(apply(s,{kind:'ADVANCE'}));l=land(s,'A1');assert.equal(l.strifeHealthLoss,0);assert.equal(l.pieces.find(p=>p.kind==='TOWN')!.strife,2);assert.equal(health(l,l.pieces.find(p=>p.kind==='TOWN')!),3);parseSpiritState(s);
});
test('Adding strife after a fear health penalty destroys damaged invaders even through a Bringer power',()=>{
 let s=chosen(1,'BRINGER');s.stage='FEAR';for(const l of s.lands){l.pieces=[];l.strifeHealthLoss=1;}makePiece(s,land(s,'A8'),'CITY');const a=land(s,'A1'),town=makePiece(s,a,'TOWN');town.damage=1;const actor=s.players[0]!.playerId;s.flags.push(`power:${actor}`);const before=s.fear;s.queue=[step('SPECIAL',actor,'A1',1,'TOKEN:ADD:strife',null,['REQUIRED'])];settle(s);s=drain(s);assert.equal(countPieces(land(s,'A1'),['TOWN']),0);assert.equal(s.fear,before+1);assert.ok(!s.flags.some(f=>f.startsWith('dream-killed:')));parseSpiritState(s);
});
test('Ravage restores strife-reduced health before Dahan counterattack',()=>{
 let s=strifeFear('unrest',3);s.queue=[];const a=land(s,'A1');a.strifeHealthLoss=1;const city=makePiece(s,a,'CITY');city.strife=1;makePiece(s,a,'DAHAN');s.queue=[step('SPECIAL',s.players[0]!.playerId,'A1',0,'RAVAGE')];settle(s);s=drain(s);const survivor=land(s,'A1').pieces.find(p=>p.kind==='CITY')!;assert.ok(survivor);assert.equal(survivor.strife,0);assert.equal(survivor.damage,2);assert.equal(health(land(s,'A1'),survivor),3);parseSpiritState(s);
});
