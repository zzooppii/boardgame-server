...
test("same room: every supported game can be selected and started by its host without ready commands", async t => {
  for (const gameType of SUPPORTED_GAME_TYPES) {
    await t.test(gameType, async t => {
      const h = await harness(t, gameType === "AVALON" ? 5 : gameType === "ARK_NOVA" ? 1 : gameType === "LIAR_GAME" ? 4 : (gameType === "SEVEN_WONDERS_DUEL" || gameType === "PATCHWORK" || gameType === "WORD_DUET" || gameType === "JAIPUR" || gameType === "GURYONGTU" || gameType === "LOST_CITIES") ? 2 : 3), before = await h.sync();
...
test("every game's finished roster returns only remaining members to the same room", async t => {
  for (const gameType of SUPPORTED_GAME_TYPES) await t.test(gameType, async t => {
    const h = await harness(t, gameType === "AVALON" ? 5 : gameType === "ARK_NOVA" ? 1 : gameType === "LIAR_GAME" ? 4 : (gameType === "SEVEN_WONDERS_DUEL" || gameType === "PATCHWORK" || gameType === "WORD_DUET" || gameType === "JAIPUR" || gameType === "GURYONGTU" || gameType === "LOST_CITIES") ? 2 : 3);
