...
test('TERRORSCAPE Feral socket: private trap assignment, map choice and reconnect keep roles and no deadlines',async t=>{
 const h=await harness(t,4);let s=await start(h);
 for(const payload of [{type:'SET_MAP',map:'CABIN'},{type:'SET_CAST',killer:'HUNTRESS',characters:['ANNA','WILLIAM','SOPHIA']}])s=h.success(await h.send(h.host,action(h,s,payload)));
 ...
 s=h.success(await h.send(h.host,action(h,s,{type:'SET_TRAPS',placements})));s=h.success(await h.send(h.host,action(h,s,{type:'BEGIN_HUNT'})));
