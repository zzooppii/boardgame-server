...
async function chooseCast(h:Harness,killer='BUTCHER',characters=['ANNA','WILLIAM','MARCO']){let s=await h.sync();const km=h.members.find(m=>m.playerId===terror(s).killerPlayerId)!;h.success(await h.send(km.client,action(h,await h.sync(km.client),{type:'SELECT_KILLER',killer})));for(let slot=0;slot<3;slot++){s=await h.sync();const member=h.members.find(m=>m.playerId===terror(s).owners[slot])!;h.success(await h.send(member.client,action(h,await h.sync(member.client),{type:'SELECT_SURVIVOR',slot,character:characters[slot]})));}return h.sync();}
async function prepare(h:Harness){let s=await h.sync();if(!terror(s).killerSelected)s=await chooseCast(h);for(const member of h.members)h.success(await h.send(member.client,action(h,await h.sync(member.client),{type:'SETUP_READY',ready:true})));return h.sync();}
async function begin(h:Harness){await start(h);const s=await prepare(h);return h.success(await h.send(h.host,action(h,s,{type:'BEGIN_HUNT'})));}
...
test('TERRORSCAPE socket: delayed older fanout cannot regress the hidden-state watermark',async t=>{
 const h=await harness(t);await start(h);const setup=await prepare(h),projector=h.server.runtime.platformSnapshotV2Projector,original=projector.project.bind(projector);
...
test('TERRORSCAPE socket: new cast, private trait drafting, forged selection and reconnect preserve base-v2',async t=>{
 ...
 s=await chooseCast(h,'MURDERER',['SOPHIA','JOHNSON','WILLIAM']);
 for(const payload of [{type:'SET_PLANS',enabled:true},{type:'SET_SEPARATE',enabled:true},{type:'SET_DIFFICULTY',difficulty:'NORMAL'}])s=h.success(await h.send(h.host,action(h,s,payload)));
 s=await prepare(h);s=h.success(await h.send(h.host,action(h,s,{type:'BEGIN_HUNT'})));
...
test('TERRORSCAPE Feral socket: private trap assignment, map choice and reconnect keep roles and no deadlines',async t=>{
 const h=await harness(t,4);let s=await start(h);
 s=h.success(await h.send(h.host,action(h,s,{type:'SET_MAP',map:'CABIN'})));s=await chooseCast(h,'HUNTRESS',['ANNA','WILLIAM','SOPHIA']);
 const placements=[{trap:'NET_A',room:'R3'},{trap:'NET_B',room:'G4'},{trap:'SKULL',room:'B2'},{trap:'BEAR',room:'B3'}];
 const member=h.members[1]!;assert.equal(h.failure(await h.send(member.client,action(h,await h.sync(member.client),{type:'SET_TRAPS',placements}))),'RULE_VIOLATION');
 s=h.success(await h.send(h.host,action(h,s,{type:'SET_TRAPS',placements})));s=await prepare(h);s=h.success(await h.send(h.host,action(h,s,{type:'BEGIN_HUNT'})));
 assert.equal(terror(s).rulesVersion,'terrorscape-feral-v2');assert.equal(terror(s).map,'CABIN');const killer=terror(s).privateState;assert.ok(killer.role==='KILLER');assert.deepEqual(killer.killer.traps,placements);
 const before=terror(await h.sync(member.client));assert.equal(JSON.stringify(before).includes('NET_A'),false);assert.equal(before.huntingTrapRooms.length,4);member.client.disconnect();const fresh=await h.connect();const resumed=h.success(await h.call(fresh,'session:resume',{credential:{...member.credential,roomCode:s.room.roomCode},lastSeenVersions:null}));assert.deepEqual(terror(resumed),before);assert.equal((await h.server.runtime.persistence.listActiveTurnDeadlines()).some(x=>x.roomId===s.room.roomId),false);
});

test('TERRORSCAPE setup socket: player-owned selections, simultaneous claims, ready state and reconnect',async t=>{
 const h=await harness(t,4);let s=await start(h);const first=h.members[1]!,second=h.members[2]!;
 assert.equal(h.failure(await h.send(h.host,action(h,s,{type:'SELECT_SURVIVOR',slot:0,character:'JOHNSON'}))),'RULE_VIOLATION');
 assert.equal(h.failure(await h.send(first.client,action(h,await h.sync(first.client),{type:'SET_ROLES',killerPlayerId:first.playerId,owners:[h.members[0]!.playerId,second.playerId,h.members[3]!.playerId]}))),'RULE_VIOLATION');
 const one=action(h,await h.sync(first.client),{type:'SELECT_SURVIVOR',slot:0,character:'JOHNSON'}),two=action(h,await h.sync(second.client),{type:'SELECT_SURVIVOR',slot:1,character:'JOHNSON'});
 const results=await Promise.all([h.send(first.client,one),h.send(second.client,two)]);assert.equal(results.filter(r=>v.parse(StateSyncWireAckSchema,r).ok).length,1);s=await h.sync();const claimed=terror(s).conditions.findIndex((c,i)=>c.character==='JOHNSON'&&terror(s).survivorSelected[i]);assert.ok(claimed===0||claimed===1);const loser=claimed===0?second:first;assert.equal(h.failure(await h.send(loser.client,action(h,await h.sync(loser.client),{type:'SELECT_SURVIVOR',slot:claimed===0?1:0,character:'JOHNSON'}))),'RULE_VIOLATION');
 const member=claimed===0?first:second;h.success(await h.send(member.client,action(h,await h.sync(member.client),{type:'SETUP_READY',ready:true})));const before=terror(await h.sync(member.client));assert.ok(before.setupReady.includes(member.playerId));member.client.disconnect();const fresh=await h.connect();const resumed=h.success(await h.call(fresh,'session:resume',{credential:{...member.credential,roomCode:s.room.roomCode},lastSeenVersions:null}));assert.deepEqual(terror(resumed).survivorSelected,before.survivorSelected);assert.deepEqual(terror(resumed).setupReady,before.setupReady);assert.deepEqual(terror(resumed).conditions,before.conditions);
 assert.equal(h.failure(await h.send(h.host,action(h,await h.sync(),{type:'BEGIN_HUNT'}))),'RULE_VIOLATION');assert.equal(h.failure(await h.send(h.host,action(h,await h.sync(),{type:'SET_CAST',killer:'BUTCHER',characters:['ANNA','WILLIAM','MARCO']}))),'INVALID_PAYLOAD');
});
