...
export function SpeakeasyPractice({onExit}:{onExit():void}) {
  ...
  const actionPanel=useRef<HTMLElement>(null),message=useRef<HTMLParagraphElement>(null),results=useRef<HTMLElement>(null);
  ...
  const visible=view?.choices.filter(c=>{
    const action=c.action;
    if(action.type!==filter)return false;
    if(district===null||action.type==='PRODUCE')return true;
    if(action.type==='BUILD'||action.type==='MOVE')return action.district===district;
    if('buildingId' in action)return view.districts[district-1]?.slots.some(b=>b?.tileId===action.buildingId);
    return false;
  })??[];
  return <main className="sp-root sp-practice" onPointerDown={()=>void audio.unlock()} onKeyDown={e=>{void audio.unlock();if(e.key==='Escape'){setSelected(null);setEndConfirm(false);setExitConfirm(false);audio.play('CANCEL');}}}>
    ...
    {!view?<section className="sp-practice-start"><SpeakeasyBuildingArt kind="SPEAKEASY"/><h2>증류소 하나, 주점 하나.<br/>이제 첫 잔을 팔 차례입니다.</h2><p>생산 → 운송 → 판매 순서로 시작해 보세요. 행동을 고르면 적용 전에 비용과 결과를 확인할 수 있습니다.</p><button type="button" disabled={busy} onClick={()=>void start()}>{busy?'테이블 준비 중…':'연습 대국 시작'}</button></section>:<>
      ...
      <div className="sp-practice-layout"><section><div className="sp-board-heading"><div><p className="sp-eyebrow">YOUR TRAINING CITY</p><h2>밤의 도시</h2></div><span>연습용 격자 · 상하좌우 연결</span></div><p>청록은 나 · 보라는 컴퓨터 · 구역을 선택하면 가능한 행동을 모아 볼 수 있습니다.</p>
        ...
        }} onClick={()=>{setDistrict(district===d.id?null:d.id);setSelected(null);audio.play('SELECT');if(window.matchMedia('(max-width: 760px)').matches){actionPanel.current?.scrollIntoView({block:'start'});actionPanel.current?.focus({preventScroll:true});}}}><span className="sp-practice-district-number">{d.id.toString().padStart(2,'0')}{d.cop&&<SpeakeasyToken kind="COP"/>}</span><span className="sp-practice-lots">{d.slots.map((b,i)=><span key={i} className={b?`sp-owner-${b.ownerId===view.viewerId?0:2} ${b.operating?'':'is-closed'}`:'is-empty'}>{b?<><SpeakeasyBuildingArt kind={b.kind}/><small>{own(b.ownerId)} · {SPEAKEASY_BUILDING_LABELS[b.kind]}</small><span>{b.protected&&<SpeakeasyToken kind="FAMILY"/>}{b.barrel&&<SpeakeasyToken kind="BARREL"/>}</span>{!b.operating&&<small>영업 중단</small>}</>:<small>+ 빈칸</small>}</span>)}</span>{view.truck.district===d.id&&<span className="sp-practice-truck"><SpeakeasyToken kind="TRUCK"/> 내 트럭 {view.truck.load}/2</span>}</button>)}</div>
        ...
        <nav aria-label="행동 종류">{(['PRODUCE','DELIVER','SELL','BUILD','PROTECT','MOVE'] as const).map(type=><button type="button" key={type} aria-pressed={filter===type} onClick={()=>{setFilter(type);setSelected(null);audio.play('SELECT');}}>{labels[type]} <small>{view.choices.filter(c=>c.action.type===type).length}</small></button>)}</nav>
        {district!==null&&<button type="button" className="sp-practice-filter" onClick={()=>setDistrict(null)}>{district}구역 선택 해제 · 전체 보기</button>}
        <div className="sp-practice-options">{visible.map((choice,i)=><button type="button" key={`${view.revision}-${i}`} disabled={busy} aria-pressed={selected===choice} onClick={()=>choose(choice)}><strong>{choice.label}</strong><small>{choice.detail}</small></button>)}{!visible.length&&<p>{view.finished?'최종 결과를 확인하세요.':view.actionsLeft===0?'이번 턴의 행동을 모두 사용했습니다. 턴을 마치면 컴퓨터가 행동합니다.':filter==='MOVE'?'현재 위치에서 이동 3칸 이내의 다른 구역을 선택하세요.':filter==='DELIVER'?'배달할 빈 주점과 주류가 필요합니다. 증류소에서 주류를 생산하고 3칸 이내의 건물에 배달하세요.':filter==='SELL'?'주류가 배달된 영업 건물이 필요합니다.':filter==='PROTECT'?'남은 조직원과 보호되지 않은 내 건물이 필요합니다.':filter==='BUILD'?'선택한 구역의 빈칸과 건설 비용이 필요합니다. 나이트클럽은 각 구역권에 하나만 지을 수 있습니다.':'생산 가능한 영업 증류소가 필요합니다.'}</p>}</div>
        {selected&&<section className="sp-practice-confirm" aria-live="polite"><h3>{selected.label}</h3><p>{selected.detail}</p><p>행동 1회 사용 · 확정 후 취소할 수 없습니다.</p><button type="button" disabled={busy} onClick={()=>void submit(selected.action)}>{busy?'처리 중…':'이 행동 확정'}</button><button type="button" disabled={busy} onClick={()=>{setSelected(null);audio.play('CANCEL');}}>취소</button></section>}
        {!view.finished&&<div className="sp-practice-end">{endConfirm?<><p>남은 행동 {view.actionsLeft}회를 사용하지 않고 턴을 마칠까요?</p><button type="button" disabled={busy} onClick={()=>void submit({type:'END_TURN'})}>남은 행동 건너뛰고 턴 종료</button><button type="button" onClick={()=>setEndConfirm(false)}>돌아가기</button></>:<button type="button" disabled={busy} onClick={()=>view.actionsLeft?setEndConfirm(true):void submit({type:'END_TURN'})}>{busy?'서버 처리 중…':'턴 마치기 → 컴퓨터 차례'}</button>}</div>}
      </aside></div>
      <section className="sp-practice-log" aria-label="진행 기록"><h2>오늘 밤의 기록</h2><p role="status" aria-live="polite">{view.log.at(-1)}</p><ol>{view.log.slice(-12).reverse().map((entry,i)=><li key={`${view.revision}-${i}`}>{entry}</li>)}</ol></section>
    </>}
