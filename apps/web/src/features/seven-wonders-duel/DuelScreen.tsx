...
function Table(props: Props & {
 ...
 <main className="du-board-center">{city(other)}{g.stage === 'DRAFT' ? <><section className="du-draft-grid">{g.draftAvailable.map(id => { const w = DUEL_WONDERS.find(w => w.id === id)!; return <button className="du-draft-wonder" key={id} onClick={() => inspectCard(id, id)}><Atlas index={w.art}/><div><span>WONDER {w.art + 1}</span><h3>{w.name}</h3><Cost cost={w.cost}/><small>{w.points}점 {w.replay ? ' · 추가 턴' : ''}</small></div></button>; })}</section>{g.settings.agora && <details className="du-draft-preview"><summary>시대 I 카드 배치 미리보기</summary>{pyramid}</details>}</> : g.board.length ? pyramid : <p className="du-empty-board">모든 시대의 카드 선택이 끝났습니다.</p>}{city(me)}</main>
 <aside className="du-empire"><section className="du-military"><div className="du-section-heading"><span>⚔ 군사력</span><b>{g.military === 0 ? '균형' : `${g.military * (seat === 0 ? 1 : -1) > 0 ? '나' : '상대'} 우세 ${Math.abs(g.military)}칸`}</b></div><div className="du-military-track">{Array.from({ length: 19 }, (_, i) => i - 9).map(n => <div key={n} className={`du-military-cell ${n === 0 ? 'center' : ''} ${Math.abs(n) === 9 ? 'capital' : ''}`}><span>{Math.abs(n) === 9 ? '♜' : g.militaryTokens.includes(n) ? g.settings.agora ? Math.abs(n) === 3 ? '+◆' : '−◆' : Math.abs(n) === 3 ? '−2' : '−5' : Math.abs(n) === 0 ? '·' : ''}</span>{g.minerva === n && <b title="미네르바">♙</b>}{g.military === n && <b className="du-conflict" title="충돌 말">⚔</b>}</div>)}</div><div className="du-track-labels"><span>{name(g.playerStates[0]!.playerId)} 수도</span><span>{name(g.playerStates[1]!.playerId)} 수도</span></div></section>
