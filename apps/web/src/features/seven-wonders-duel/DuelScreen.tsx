import { AgoraBoard, AgoraDetails } from './AgoraBoard.js';
import { useEffect, useRef, useState, type CSSProperties, type ReactNode } from 'react';
import { DUEL_DEFAULT_SETTINGS, DUEL_CARDS, DUEL_WONDERS, DUEL_GODS, PROTOCOL_VERSION, type DuelProjection, type DuelClientCommand, type DuelOption } from '@hangul-rummikub/shared';
...
import { Atlas, Card, DetailArt, definition } from './art.js';
import { useDuelSound } from './sound.js';
import './duel.css';
import { duelInspectionOptions } from './inspection.js';
import { WonderCard, WonderDetails } from './WonderCard.js';
import { MilitaryView, militarySummary } from './MilitaryView.js';
import { BuildingReference } from './BuildingReference.js';
import { TurnTimer } from './TurnTimer.js';
import { CityPanel } from './CityPanel.js';
import { BuildingDetails, BuildingPreview } from './BuildingDetails.js';
type Props = {
...
function Modal({ children, close, label, wide = false }: {
    wide?: boolean;
    children: ReactNode;
    close(): void;
    label: string;
    ...
    return <dialog ref={ref} className={`du-modal ${wide ? 'du-modal-wide' : ''}`} aria-label={label} onCancel={close} onClick={e => {
...
export default function DuelScreen(props: Props) {
 ...
 {g ? <Table key={g.gameId + props.snapshot.self.playerId} {...props} game={g} canAct={can} busy={busy} choose={o => {
            ...
            }} selectSound={() => sound.play('select')}/> : <div className="du-lobby"><div className="du-lobby-scene"><Atlas index={8}/><div><span>BUILD A CIVILIZATION</span><h2>두 문명.<br />하나의 역사.</h2><p>당신의 다음 선택이<br />고대 세계를 바꿉니다.</p></div><div className="du-lobby-god"><Atlas index={8} gods/></div></div><div className="du-lobby-panel"><span className="du-eyebrow">THE TABLE IS SET</span><h2>어떤 역사를 만들까요?</h2><p>불가사의, 과학, 군사력. 그리고 신들의 힘과 원로원의 영향력까지.</p><div className="du-expansions">{(['pantheon', 'agora'] as const).map((key, i) => <label key={key}><Atlas index={i ? 14 : 12}/><span><strong>{i ? 'Agora' : 'Pantheon'}</strong><small>{i ? '의원 · 음모 · 정치 승리' : '신화 · 판테온 · 대신전'}</small></span><input type="checkbox" checked={'settings' in props.snapshot.room ? props.snapshot.room.settings[key] : true} disabled={!can || !start.isHost} onChange={e => { const settings = 'settings' in props.snapshot.room ? props.snapshot.room.settings : DUEL_DEFAULT_SETTINGS; void send({ protocolVersion: PROTOCOL_VERSION, kind: 'duel:configure', requestId: createRequestId(), expectedRoomRevision: props.snapshot.versions.roomRevision, payload: { ...settings, [key]: e.target.checked } }); }}/></label>)}</div><fieldset className="du-timer-settings" disabled={!can || !start.isHost}><legend>턴 제한시간</legend><div>{([0, 30, 60, 90] as const).map(seconds => <label key={seconds}><input type="radio" name="duel-turn-duration" value={seconds} checked={('settings' in props.snapshot.room ? props.snapshot.room.settings.turnDurationSeconds : 60) === seconds} onChange={() => { const settings = 'settings' in props.snapshot.room ? props.snapshot.room.settings : DUEL_DEFAULT_SETTINGS; void send({ protocolVersion: PROTOCOL_VERSION, kind: 'duel:configure', requestId: createRequestId(), expectedRoomRevision: props.snapshot.versions.roomRevision, payload: { ...settings, turnDurationSeconds: seconds } }); }} />{seconds === 0 ? '제한 없음' : `${seconds}초`}</label>)}</div><p>제한 없음은 자동 진행 없이 기다립니다.<br />시간 초과 시 카드 버리기 또는 필수 효과 자동 선택.<br />연속 효과는 같은 시간을 쓰며, 추가 턴에는 새 시간이 주어집니다.</p></fieldset><div className="du-lobby-players">{props.snapshot.room.players.map((p, i) => <div key={p.playerId}><b>{i + 1}</b><span>{p.nickname}<small>{p.connectionStatus === 'CONNECTED' ? '접속 중' : '재접속 대기'}</small></span>{p.isHost && <small>방장</small>}</div>)}{props.snapshot.room.players.length === 1 && <button onClick={props.onCopy}>+ 상대 초대하기</button>}</div><button className="du-primary" disabled={!start.canStart} onClick={props.onStart}>문명 건설 시작 →</button><small>{start.guidance}</small><p className="du-lobby-note">2인 · 기본판 또는 확장 조합 · 턴 시간 설정 가능<br />카드를 눌러 확대하고, 행동을 선택한 뒤 확정하세요.</p></div></div>}
 </section>;
}
function Table(props: Props & {
    ...
    const [inspect, setInspect] = useState<{
        id: string | null;
        source: string;
    } | null>(null), [option, setOption] = useState<string | null>(null), [reference, setReference] = useState(false), [ledger, setLedger] = useState(false), [militaryOpen, setMilitaryOpen] = useState(false), [agoraChamber, setAgoraChamber] = useState<number | null>(null);
    ...
    const actions = inspect ? duelInspectionOptions(options, inspect) : [];
    const maxX = Math.max(...g.slots.map(s => s.x)) + 1, maxY = Math.max(0, ...g.board.map(c => g.slots.find(s => s.index === c.slot)?.y ?? 0));
    const wonderList = (playerId: string) => <div className="du-wonder-grid">{g.wonders.filter(w => w.owner === playerId).map(w => <WonderCard key={`${w.id}-${g.gameRevision}`} wonder={DUEL_WONDERS.find(d => d.id === w.id)!} built={w.built} removed={w.removed} onInspect={() => inspectCard(w.id, w.id)} />)}</div>;
    const city = (p: typeof me) => <CityPanel player={p} name={name(p.playerId)} own={p.playerId === self} settings={g.settings} onInspect={inspectCard} />;
    const pyramid = <div><span className="du-scroll-hint">↔ 배치를 좌우로 밀어 모든 카드를 확인하세요</span><div className="du-pyramid-scroll"><div className="du-pyramid" style={{ '--du-columns': maxX, width: `${maxX * 103 + 30}px`, height: `${maxY * 69 + 160}px` } as CSSProperties}>{g.board.map(c => { const d = DUEL_CARDS.find(d => d.id === c.definitionId), slot = g.slots.find(s => s.index === c.slot)!, available = !slot.coveredBy.some(i => g.board.some(b => b.slot === i)), build = options.find(o => o.group === 'BUILD' && o.sourceId === c.tileId); return <Card tooltip={d ? <BuildingPreview card={d} quotedCost={build?.cost} /> : undefined} key={c.tileId} id={c.definitionId} back={c.back} available={available} style={{ left: 15 + slot.x * 103, top: slot.y * 69, zIndex: slot.y + 1 }} onClick={() => inspectCard(c.definitionId, c.tileId)} tag={slot.mythology ? '✦ 신화' : slot.offering ? `공물 ${slot.offering}` : available ? (build ? `${build.cost} ●` : '선택 가능') : undefined}/>; })}</div></div></div>;
 ...
 {g.phase === 'FINISHED' && <section className="du-result"><span className="du-eyebrow">{({SCORED:'문명 점수 승리',MILITARY:'군사 승리',SCIENCE:'과학 승리',POLITICAL:'정치 승리',CANCELLED:'게임 취소'} as const)[g.result.reason]}</span><h2>{g.result.reason === 'CANCELLED' ? '이번 게임이 취소되었습니다' : g.result.winnerPlayerIds.length === 2 ? '두 문명의 공동 승리' : `${name(g.result.winnerPlayerIds[0]!)}님의 승리`}</h2><div>{g.result.scores.map(s => <article key={s.playerId}><strong>{name(s.playerId)}</strong><b>{s.total}<small>점</small></b><p>건물 {s.blue + s.green + s.yellow + s.guild} · 불가사의 {s.wonders}<br />진보 {s.progress} · 코인 {s.coins} · 군사 {s.military}<br />신·대신전 {s.gods + s.temples} · 원로원 {s.senate}</p></article>)}</div>{props.snapshot.room.players.find(p => p.playerId === self)?.isHost && <button disabled={!props.canAct} className="du-primary" onClick={props.onRematch}>같은 방에서 다시 플레이 →</button>}</section>}
 <div className="du-status" role="status"><span className={mine ? 'active' : ''}/><strong>{g.phase === 'FINISHED' ? '게임 종료' : mine ? '당신의 선택' : '상대의 선택'}</strong><span>{g.decision?.label ?? '결과를 확인하세요'}</span>{mine && (g.stage === 'RESOLVING' || g.stage === 'NEXT_AGE') && <a href="#du-next-choice">선택 목록으로 ↓</a>}{g.phase === 'PLAYING' && <TurnTimer deadlineAt={g.deadlineAt} serverTime={props.snapshot.serverTime} connected={props.connected} />}</div>
 {g.settings.pantheon && <section className="du-pantheon"><div className="du-side-title"><span>✦</span><strong>PANTHEON</strong><small>{g.age === 1 ? '신들을 초대하세요' : '카드 대신 신 활성화'}</small></div>{g.pantheon.map((slot, i) => <button key={i} className={`du-god ${slot.godId ? 'revealed' : ''}`} onClick={() => inspectCard(slot.godId, `pantheon-${i}`)}><span className="du-god-slot">{i + 1}</span>{slot.godId ? <Atlas index={DUEL_GODS.findIndex(d => d.id === slot.godId)} gods/> : <span className="du-god-seal">{slot.occupied ? '✦' : '◇'}</span>}<strong>{slot.godId ? definition(slot.godId)?.name : slot.occupied ? '비공개 신' : '빈 자리'}</strong><small>나 {slot.costs[seat]} ● · 상대 {slot.costs[1 - seat]} ●</small></button>)}</section>}
 {g.settings.agora && <AgoraBoard chambers={g.decrees} self={self} mine={me.influence} theirs={other.influence} onInspect={setAgoraChamber} />}
 ...
 <main className="du-board-center">{city(other)}{g.stage === 'DRAFT' ? <><p className="du-draft-help">마우스를 올리면 전체 효과를 볼 수 있습니다. 모바일에서는 카드를 누르세요. 선택 확정 전에는 진행되지 않습니다.</p><section className="du-draft-grid" aria-label="선택 가능한 불가사의">{g.draftAvailable.map(id => <WonderCard key={`${id}-${g.gameRevision}`} wonder={DUEL_WONDERS.find(w => w.id === id)!} draft onInspect={() => inspectCard(id, id)} />)}</section>{(g.settings.agora || g.settings.pantheon) && <details className="du-draft-preview"><summary>시대 I 카드 배치 미리보기</summary>{pyramid}</details>}</> : g.board.length ? pyramid : <p className="du-empty-board">모든 시대의 카드 선택이 끝났습니다.</p>}{city(me)}</main>
 <aside className="du-empire"><section className="du-military"><button type="button" className="du-military-open" aria-label="군사력 트랙 확대 및 효과 보기" aria-haspopup="dialog" onClick={() => setMilitaryOpen(true)}><div className="du-section-heading"><span>⚔ 군사력</span><b>{militarySummary(g.military, seat)}</b></div><div className="du-military-track">{Array.from({ length: 19 }, (_, i) => i - 9).map(n => <div key={n} className={`du-military-cell ${n === 0 ? 'center' : ''} ${Math.abs(n) === 9 ? 'capital' : ''}`}><span>{Math.abs(n) === 9 ? '♜' : g.militaryTokens.includes(n) ? g.settings.agora ? Math.abs(n) === 3 ? '+◆' : '−◆' : Math.abs(n) === 3 ? '−2' : '−5' : Math.abs(n) === 0 ? '·' : ''}</span>{g.minerva === n && <b title="미네르바">♙</b>}{g.military === n && <b className="du-conflict" title="충돌 말">⚔</b>}</div>)}</div><div className="du-track-labels"><span>{name(g.playerStates[0]!.playerId)} 수도</span><span>{name(g.playerStates[1]!.playerId)} 수도</span></div><span className="du-military-open-hint">ⓘ 눌러서 구간별 효과 보기</span></button></section>
 ...
 {inspect && <Modal close={() => { setInspect(null); setOption(null); }} label="카드 확대 및 행동 선택">{g.phase === 'PLAYING' && <div className="du-modal-timer"><TurnTimer deadlineAt={g.deadlineAt} serverTime={props.snapshot.serverTime} connected={props.connected} /></div>}{inspect.id ? <><DetailArt id={inspect.id}/><div className="du-detail-copy">{(() => { const d = definition(inspect.id!); return d && 'cost' in d && !('color' in d) ? <WonderDetails wonder={d} draft={g.stage === 'DRAFT'} /> : d && 'cost' in d ? <BuildingDetails card={d} owned={g.playerStates.some(p => p.buildings.some(c => c.tileId === inspect.source))} quotedCost={options.find(o => o.group === 'BUILD' && o.sourceId === inspect.source)?.cost} /> : d && 'text' in d ? <p>{d.text}</p> : null; })()}</div></> : <div className="du-detail-copy"><h2>{actions.length || selected ? '행동 선택' : '비공개 정보'}</h2><p>{actions.length || selected ? '선택할 위치나 효과를 확인하세요.' : '규칙상 공개되는 시점에 내용을 확인할 수 있습니다.'}</p></div>}<div className="du-detail-actions">{actions.map(o => <button key={o.id} disabled={!can} className={option === o.id ? 'selected' : ''} onClick={() => { setOption(o.id); props.selectSound(); }}><strong>{o.label}</strong>{o.detail && <small>{o.detail}</small>}</button>)}{selected && <div className="du-confirm"><p><strong>{selected.label}</strong></p><button className="du-primary" disabled={!can} onClick={() => props.choose(selected)}>{props.busy ? '처리 중…' : '이 선택 확정 →'}</button></div>}{!actions.length && !selected && <p className="du-help">{mine ? '이 대상에 지금 가능한 행동이 없습니다.' : '다른 플레이어의 선택을 기다리고 있습니다.'}</p>}</div></Modal>}
 {agoraChamber !== null && g.decrees.find(ch => ch.chamber === agoraChamber) && <Modal close={() => setAgoraChamber(null)} label="원로원 의회와 법령"><AgoraDetails chamber={g.decrees.find(ch => ch.chamber === agoraChamber)!} self={self} mine={me.influence} theirs={other.influence} /></Modal>}
 {militaryOpen && <Modal close={() => setMilitaryOpen(false)} label="군사력 트랙과 효과"><MilitaryView game={g} seat={seat} myName={name(self)} opponentName={name(other.playerId)} /></Modal>}
 {reference && <Modal close={() => setReference(false)} label="건물 연계 참고표" wide><BuildingReference pantheon={g.settings.pantheon} agora={g.settings.agora} onInspect={id => { setReference(false); inspectCard(id, id); }} /></Modal>}
