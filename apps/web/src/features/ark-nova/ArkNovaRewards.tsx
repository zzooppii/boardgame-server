...
import {type ArkSoloView,type ArkSoloCommand} from '@hangul-rummikub/shared';
...
export function ArkNovaRewards({state:s,disabled,onCommand:send}:{state:ArkSoloView;disabled:boolean;onCommand(command:ArkSoloCommand):void}) {
  ...
  return <section aria-label="보너스 선택"><h2>동물원 보너스</h2><p>원하는 순서대로 보너스를 받으세요.</p>{options.map(option=>{
    ...
    return <div key={option.id} className="ark-live-reward"><h3>{labels[option.kind]??option.kind}</h3>
      {upgrade?<div className="ark-upgrade-options">{s.actions.filter(a=>!a.upgraded&&s.actions.filter(c=>c.upgraded).length<4).map(a=><ArkNovaUpgradeChoice key={a.kind} action={a.kind} disabled={disabled} onChoose={()=>take({kind:'UPGRADE',action:a.kind})}/>)}</div>:card?<><button disabled={disabled||s.deckCount===0} onClick={()=>take({kind:'CARD',cardId:null})}>덱에서 가져오기</button><ArkNovaCardRow cards={arkReachableDisplayCards(s)} disabled={disabled} onSelect={id=>take({kind:'CARD',cardId:id})}/></>:<button disabled={disabled} onClick={()=>take({kind:'NONE'})}>받기</button>}
