...
export function ArkNovaEffects({state:s,disabled,onSelect,placement,cell,housingId,onUniqueCard,onBuilding,onRotate,onReflect,onAnimalCard,onClearHousing,selectedBuildingKind,onChooseHousing}:{
  ...
  if(kind==='INTERACTION'&&guide.interaction&&s.table){
  ...
  }else if(['HOLLYWOOD','MOVE_1_TWICE','GAIN','DRAW','BREAK','SPONSOR_TOKENS','SPONSOR_MAGNET'].includes(kind))controls=button('효과 적용',{kind:'NONE'});
  ...
  else if(['MOVE_ACTION','MULTIPLIER','EXTRA_ACTION'].includes(kind))controls=<>{guide.actions.map(action=>kind==='EXTRA_ACTION'?button(action==='TAKE_X'?'X 토큰 받기':ARK_ACTION_LABELS[action],{kind:'ACTION',action}):action==='TAKE_X'?null:kind==='MOVE_ACTION'?guide.slots.map(slot=>button(`${ARK_ACTION_LABELS[action]} ${s.actions.findIndex(card=>card.kind===action)+1}번 → ${slot}번`,{kind:'MOVE',action,slot})):button(ARK_ACTION_LABELS[action],{kind:'MULTIPLIER',action}))}{(kind==='MOVE_ACTION'||kind==='EXTRA_ACTION')&&skip()}</>;
    ...
    controls=<><ArkNovaAssociationBenefits mapId={s.mapId} kind="PARTNER" count={s.partners.length}/>{available.map(continent=>button(ARK_TAG_LABELS[continent]??continent,{kind:'PARTNER',continent}))}{!available.length&&button('대상 없음 · 계속',{kind:'NONE'})}</>;
  } else if(kind==='FREE_UNIVERSITY')controls=<><ArkNovaAssociationBenefits mapId={s.mapId} kind="UNIVERSITY" count={s.universities.length}/>{ARK_SOLO_UNIVERSITIES.filter(u=>s.universitySupply.includes(u)).map(university=>button(ARK_UNIVERSITY_LABELS[university],{kind:'UNIVERSITY',university}))}{!s.universitySupply.length&&button('대상 없음 · 계속',{kind:'NONE'})}</>;
