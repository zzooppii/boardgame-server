...
import {arkSponsorSelectionAdvice,arkReachableDisplayCards} from './action-controls.js';
...
export function ArkNovaEffects({state:s,disabled,onSelect,placement,cell,housingId,onUniqueCard,onBuilding,onRotate,onReflect}:{
  ...
  if(['GAIN','DRAW','SPONSOR_TOKENS','SPONSOR_MAGNET'].includes(kind))controls=button('효과 적용',{kind:'NONE'});
    ...
    controls=<><p>{kind==='WAZA_PLAY'?'동물과 입주할 우리를 선택하세요.':'손패의 후원자를 선택하고 후원 등급만큼 돈을 냅니다. 고유 건물이 있다면 지도에서 위치도 선택하세요.'}</p>{row(cards)}{kind==='PAID_SPONSOR'&&cards.length===0&&<p>손패에 후원자가 없습니다. 이 효과를 포기할 수 있습니다.</p>}{sponsorAdvice&&<div className="ark-admission-guide" aria-label="효과 후원자 사용 조건"><strong>지불할 돈 {sponsorAdvice.price} · 보유 {s.money}</strong>{sponsorAdvice.issues.map(text=><p className="ark-admission-missing" key={text}>{text}</p>)}</div>}{kind==='PAID_SPONSOR'&&<button disabled={disabled} onClick={onRotate}>고유 건물 회전 ↻</button>}{button('카드 사용',{kind:kind==='WAZA_PLAY'?'ANIMAL':'SPONSOR',card:{cardId:chosen[0]??'',housingId:kind==='WAZA_PLAY'?housingId:null,...(placement&&kind==='PAID_SPONSOR'&&cards.some(c=>c.cardId===chosen[0]&&Object.hasOwn(ARK_UNIQUE_BUILDINGS,c.key))?{uniquePlacement:{anchor:placement.anchor,rotation:placement.rotation}}:{})}},chosen.length!==1||!!sponsorAdvice?.issues.length)}{skip()}</>;
