...
import { ArnakGuardianPreview } from './ArnakGuardianPreview.js';
import { ArnakResearchTrack } from './ArnakResearchTrack.js';
import { ArnakResearchDetail } from './ArnakResearchDetail.js';
import { arnakOfferLabel } from './offer-list.js';
import { ArnakOfferList } from './ArnakOfferList.js';
import { ArnakActionHints, ArnakTravelStock, ArnakTravelExpiry } from './ArnakActionHints.js';
...
function Table(props: Props & {
    ...
    const offers = g.privateState.offers, available = new Set(offers.map(o => o.targetId)), canAct = !finished && g.activePlayerId === self && props.connected && !busy && !props.pending && !uncertain, offer = offers.find(o => o.id === selected), shown = target ? offers.filter(o => o.targetId === target || target === 'idol' && o.kind === 'END') : offers.filter(o => g.pendingLabels.length ? !['CARD', 'ASSISTANT', 'BOON', 'IDOL'].includes(o.kind) : o.kind === 'END' || o.kind === 'PASS' || o.kind === 'IDOL' || o.kind === 'ASSISTANT' || o.kind === 'BOON'), name = (id: string) => props.snapshot.room.players.find(p => p.playerId === id)?.nickname ?? '탐험가';
    const idolContext = offers.some(o => o.kind === 'IDOL') ? { remaining: me.idols - me.idolSlots, slotPoints: me.idolSlots + 1 } : undefined;
 ...
 <aside className="ar-action-panel" id="ar-actions"><div className="ar-section-title"><h2>{g.stage === 'CLEANUP' ? '라운드 정리' : '탐험 수첩'}</h2><button onClick={() => { setTarget(null); setSelected(null); }}>전체</button></div>{target && [...g.market, ...g.privateState.hand, ...g.privateState.revealedCards].some(c => c.tileId === target) && <div className="ar-card-inspector">{(() => { const c = [...g.market, ...g.privateState.hand, ...g.privateState.revealedCards].find(c => c.tileId === target)!, d = arnakCard(c.definitionId); return <><h3>{d.name}</h3><button type="button" className="ar-inspect-button" onClick={() => setDetailCardId(c.tileId)}>일러스트·효과 크게 보기 ↗</button><p>{d.effects.map(describe).join(' · ')}</p><small>이동: {d.travel.map(t => ARNAK_TRAVEL_NAMES[t]).join(' + ')} · {d.free ? '자유 행동' : '주 행동'}{d.type === 'artifact' ? ' · 손패 사용 시 석판 1' : ''}</small>{d.passEffects.length > 0 && <p>대신 패스하고 {d.passEffects.map(describe).join(' · ')}</p>}</>; })()}</div>}{g.sites.some(site=>site.id===target)&&<div className="ar-card-inspector">{(()=>{const site=g.sites.find(site=>site.id===target)!;return <><ArnakWorldArt definitionId={site.definitionId} expanded/><h3>{site.definitionId?arnakSite(site.definitionId).name:'미발견 유적'}</h3><p>이동: {site.travel.map(t=>t.map(k=>ARNAK_TRAVEL_NAMES[k]).join(' + ')).join(' / ')}</p>{site.definitionId?<p>{arnakSite(site.definitionId).effects.map(describe).join(' · ')}</p>:<p>발견 비용: 나침반 {site.level===1?3:6} · 우상 {site.idolCount}개</p>}{site.guardianId&&<ArnakGuardianPreview definitionId={site.guardianId}/>}</>;})()}</div>}{(() => {
     ...
     return null;
 })()}{(() => { const receipt = [...g.history].reverse().find(h => h.playerId === self && h.round === g.round && h.text.includes('연구 보상 획득:')); return receipt && <p className="ar-research-receipt" role="status"><b>최근 연구 보상 처리</b><br/>{receipt.text.split(' · 연구 보상 획득:')[1]}</p>; })()}<ArnakResearchDetail target={target} templeSupply={g.templeSupply}/>{g.stage === 'CLEANUP' && g.activePlayerId === self && !finished && <ArnakCleanup hand={g.privateState.hand} keep={g.privateState.keep} selected={target} disabled={!canAct} onSelect={select}/>}<p className="ar-action-help">{!canAct ? finished ? '탐험의 결과를 확인하세요.' : !props.connected ? '연결 복구 후 행동할 수 있습니다.' : busy ? '서버에서 행동을 확인하고 있습니다.' : uncertain ? '처리 결과를 다시 확인해주세요.' : `${name(g.activePlayerId)}의 선택을 기다립니다.` : target ? '1. 지불 방법 선택 → 2. 아래에서 확정' : g.pendingLabels.length ? '추가 효과를 처리하세요.' : '지도·카드·연구 칸을 눌러 행동을 고르세요.'}</p>{g.privateState.revealedCards.length > 0 && <div className="ar-revealed">{g.privateState.revealedCards.map(c => <Card key={c.tileId} card={c} selected={target === c.tileId} available onSelect={() => select(c.tileId)}/>)}</div>}{!target && g.stage === 'ACTION' && !g.pendingLabels.length && !finished && g.activePlayerId === self && <ArnakActionBrowser key={g.gameRevision} offers={offers} names={actionNames} disabled={!canAct} onSelect={select}/>}<ArnakActionHints hints={g.privateState.blockedActions} target={target}/><ArnakOfferList key={`${g.gameRevision}:${target ?? ''}`} offers={shown} idol={idolContext} hand={g.privateState.hand} selected={selected} disabled={!canAct} onSelect={id => { setSelected(id); if (id) props.play('SELECT'); }}/>{!shown.length && !(g.privateState.blockedActions ?? []).some(h => h.targetId === target) && <p className="ar-muted">{target ? '현재 실행 가능한 행동이 없습니다. 필요한 자원·이동 카드·주 행동 사용 여부를 확인하세요.' : '선택 가능한 대상을 지도와 손패에서 찾을 수 있습니다.'}</p>}{offer && <div className="ar-confirm"><b>{arnakOfferLabel(offer, idolContext)}</b><p>{offer.kind === 'END' ? '이 선택으로 이번 차례를 마칩니다.' : arnakCostText(offer.cost)}</p>{offer.kind === 'IDOL' && idolContext && <p className="ar-idol-confirm">우상 1개를 슬롯에 놓고 <b>{offer.label}</b> 효과를 받습니다.<br/>빈 슬롯 {idolContext.slotPoints}점은 포기하며, 우상 자체의 3점은 유지됩니다.</p>}{offer.kind === 'END' && idolContext && <p className="ar-idol-confirm">우상은 사용하지 않고 보관합니다. 슬롯 점수도 그대로 유지하며 이번 차례를 마칩니다.</p>}{g.stage === 'CLEANUP' && offer.kind === 'PASS' && <p className="ar-cleanup-note">보관 {g.privateState.hand.filter(c => g.privateState.keep.includes(c.tileId)).length}장 · 내려놓기 {g.privateState.hand.filter(c => !g.privateState.keep.includes(c.tileId)).length}장<br/>이 선택으로 이번 라운드를 마칩니다.</p>}{(offer.travelUsed?.length ?? 0) > 0 && <div className="ar-travel-payment"><b>획득한 이동에서 사용</b><ArnakTravel travel={offer.travelUsed ?? []}/></div>}{['END', 'PASS'].includes(offer.kind) && <ArnakTravelExpiry travel={g.privateState.travel ?? []}/>} {offer.cards.length > 0 && <p className="ar-payment-summary">지불할 카드: {offer.cards.map(id => arnakCard(g.privateState.hand.find(c => c.tileId === id)!.definitionId).name).join(' · ')}<small>이 카드는 효과를 사용하지 않고 내려놓습니다.</small></p>}<button className="ar-primary" onClick={() => void send(offer)} disabled={!canAct}>{busy ? '확인 중…' : '선택 확정 →'}</button><small>{offer.kind === 'END' ? '확정하면 차례가 넘어갑니다.' : offer.kind === 'IDOL' ? '확정하면 우상 슬롯과 선택한 효과가 적용됩니다.' : '확정 시 비용과 카드가 적용됩니다.'}</small></div>}{error && <p className="ar-alert" role="alert">{error}</p>}{uncertain && <button disabled={busy || !props.connected} onClick={() => { const saved = request.current; const o = offers.find(o => o.id === saved?.payload.actionId); if (o)
        void send(o); }}>동일 요청 처리 결과 다시 확인</button>}<ArnakJournal history={g.history} players={props.snapshot.room.players}/></aside></div>
 <section className="ar-camp" id="ar-hand"><div className="ar-camp-heading"><div><span className="ar-kicker">YOUR EXPEDITION</span><h2>{name(self)}의 야영지</h2><p>탐험가 {me.workers}/2 · 덱 {me.deckCount}장 · 사용한 카드 {me.played.length}장</p></div><span className="ar-resources ar-camp-resources" aria-label="내 보유 자원"><ArnakResourceChips value={me.resources} names showZero/></span><div className="ar-idol-slots"><button onClick={() => select('idol')}>𓅓 우상 {me.idols}</button>{[1, 2, 3, 4].map((v, i) => <span key={v} className={me.idolSlots > i ? 'used' : ''}>{me.idolSlots > i ? '𓅓' : v + ' VP'}</span>)}</div></div>{g.privateState.travel && <ArnakTravelStock travel={g.privateState.travel}/>}<div className="ar-hand" aria-label="내 손패">{g.privateState.hand.map(c => <div key={c.tileId}>{g.stage === 'CLEANUP' && g.privateState.keep.includes(c.tileId) && <span className="ar-keep">다음 라운드 보관</span>}<Card card={c} payment={offer?.cards.includes(c.tileId) ?? false} selected={target === c.tileId} available={available.has(c.tileId)} onSelect={() => select(c.tileId)}/></div>)}</div><div className="ar-assistants ar-owned-abilities" aria-label="내 조수와 수호자">{me.assistants.map(a => <button key={a.definitionId} onClick={() => select(a.definitionId)} className={a.ready ? 'ready' : 'spent'} aria-pressed={target === a.definitionId}><ArnakWorldArt definitionId={a.definitionId}/><span>{a.gold ? '★ ' : '☆ '}{arnakAssistant(a.definitionId).name}<small>{a.ready ? '사용 가능' : '사용 완료'} · {arnakAssistant(a.definitionId).free ? '자유 행동' : '주 행동'}</small><small className="ar-ability-effect">{(a.gold ? arnakAssistant(a.definitionId).gold : arnakAssistant(a.definitionId).silver).map(describe).join(' · ')}</small></span></button>)}{me.guardians.map(g => <button key={g.definitionId} onClick={() => select(g.definitionId)} className={g.used ? 'spent' : 'ready'} aria-pressed={target === g.definitionId}><ArnakWorldArt definitionId={g.definitionId}/><span>{arnakGuardian(g.definitionId).name}<small>{g.used ? '축복 사용 완료' : '축복 사용 가능'} · 자유 행동 · 5 VP</small><small className="ar-ability-effect">{arnakGuardian(g.definitionId).boon.map(describe).join(' · ')}</small></span></button>)}</div><details className="ar-supply"><summary>고용 가능한 조수</summary><p className="ar-supply-note">공급 더미 맨 위의 조수입니다. 고용하면 은색 능력으로 시작합니다.</p><div className="ar-assistants ar-assistant-market">{g.assistantSupply.flatMap(stack => stack.slice(0, 1)).map(id => {
