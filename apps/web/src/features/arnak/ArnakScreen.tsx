...
function Card({ card, selected = false, available = false, payment = false, onSelect }: {
    card: ArnakCard;
    selected?: boolean;
    available?: boolean;
    payment?: boolean;
    onSelect(): void;
}) { const d = arnakCard(card.definitionId); return <button className={`ar-card ${d.type} ${selected ? 'selected' : ''} ${available ? 'available' : ''} ${payment ? 'payment' : ''}`} onClick={onSelect} aria-pressed={selected} aria-label={`${d.name}, ${d.points}점, ${d.effects.map(describe).join(', ')}`}><div className="ar-card-title"><span>{d.travel.map(t => ({ foot: '♟', car: '▰', boat: '◒', plane: '✈' })[t]).join(' ')}</span><strong>{d.name}</strong></div><ArnakCardArt definitionId={d.id}/><span className="ar-card-type">{payment ? '지불 예정' : d.type === 'artifact' ? '유물' : d.type === 'item' ? '아이템' : d.type === 'fear' ? '공포' : '기본 카드'}</span><div className="ar-card-effect">{d.free && <b className="ar-lightning">ϟ </b>}{d.effects.length ? d.effects.map(describe).join(' · ') : '이동에 사용하거나 제거하세요.'}{d.exileSelf && <small>사용 후 이 카드 제거</small>}</div><div className="ar-card-foot"><span>{d.type === 'item' ? '◉ ' + d.cost : d.type === 'artifact' ? '✥ ' + d.cost : d.type === 'fear' ? '공포' : '시작 카드'}</span><b>{d.points} VP</b></div></button>; }
...
function Table(props: Props & {
 ...
 <section className="ar-camp" id="ar-hand"><div className="ar-camp-heading"><div><span className="ar-kicker">YOUR EXPEDITION</span><h2>{name(self)}의 야영지</h2><p>탐험가 {me.workers}/2 · 덱 {me.deckCount}장 · 사용한 카드 {me.played.length}장</p></div><Resources value={me.resources}/><div className="ar-idol-slots"><button onClick={() => select('idol')}>𓅓 우상 {me.idols}</button>{[1, 2, 3, 4].map((v, i) => <span key={v} className={me.idolSlots > i ? 'used' : ''}>{me.idolSlots > i ? '𓅓' : v + ' VP'}</span>)}</div></div><div className="ar-hand" aria-label="내 손패">{g.privateState.hand.map(c => <div key={c.tileId}>{g.stage === 'CLEANUP' && g.privateState.keep.includes(c.tileId) && <span className="ar-keep">다음 라운드 보관</span>}<Card card={c} payment={offer?.cards.includes(c.tileId) ?? false} selected={target === c.tileId} available={available.has(c.tileId)} onSelect={() => select(c.tileId)}/></div>)}</div><div className="ar-assistants" aria-label="내 조수와 수호자">{me.assistants.map(a => <button key={a.definitionId} onClick={() => select(a.definitionId)} className={a.ready ? 'ready' : ''}><ArnakWorldArt definitionId={a.definitionId}/><span>{a.gold ? '★ ' : '☆ '}{arnakAssistant(a.definitionId).name}<small>{a.ready ? '사용 가능' : '사용 완료'}</small></span></button>)}{me.guardians.map(g => <button key={g.definitionId} onClick={() => select(g.definitionId)}><ArnakWorldArt definitionId={g.definitionId}/><span>{arnakGuardian(g.definitionId).name}<small>{g.used ? '축복 사용 완료' : '축복 사용 가능'} · 5 VP</small></span></button>)}</div><details className="ar-supply"><summary>고용 가능한 조수</summary><div className="ar-assistants">{g.assistantSupply.flatMap(stack => stack.slice(0, 1)).map(id => <button key={id} onClick={() => select(id)}><ArnakWorldArt definitionId={id}/><span>{arnakAssistant(id).name}<small>은색·금색 능력 확인</small></span></button>)}</div></details><details><summary>사용한 카드 확인</summary><div className="ar-played">{me.played.map(c => <button key={c.tileId} onClick={() => setDetailCardId(c.tileId)}>{arnakCard(c.definitionId).name} ↗</button>)}</div></details></section>
