...
import type { ArkNovaActCommand } from "@hangul-rummikub/shared";
...
import type { GreatKingdomClientCommand } from "@hangul-rummikub/shared";
...
import type { PatchworkClientCommand } from "@hangul-rummikub/shared";
import type { ArnakClientCommand } from "@hangul-rummikub/shared";
import type { MarsClientCommand } from "@hangul-rummikub/shared";
import type { DuelClientCommand } from "@hangul-rummikub/shared";
...
import type { AvalonClientCommand } from "@hangul-rummikub/shared";
...
export type LobbyAppState = Readonly<{
  ...
  actArkNova: (command: ArkNovaActCommand) => Promise<void>;
  ...
  actGreatKingdom: (command: GreatKingdomClientCommand) => Promise<void>;
  ...
  actPatchwork: (command: PatchworkClientCommand) => Promise<void>;
  actArnak: (command: ArnakClientCommand) => Promise<void>;
  actMars: (command: MarsClientCommand) => Promise<void>;
  actDuel: (command: DuelClientCommand) => Promise<void>;
  ...
  actAvalon: (command: AvalonClientCommand) => Promise<void>;
...
export function useLobbyApp(): LobbyAppState {
  ...
  async function actArkNova(command: ArkNovaActCommand): Promise<void> {
    const client = clientRef.current, session = storedSessionForCurrentRoute();
    if (!client?.connected || session === null || sessionReplacedRef.current || compatibleSnapshotRef.current?.kind !== "PLATFORM_V2_ARK_NOVA") throw new Error("연결을 확인하고 다시 시도해주세요.");
    const ack = await client.actArkNova(command);
    if (clientRef.current !== client || sessionReplacedRef.current || storedSessionForCurrentRoute()?.playerId !== session.playerId ||
      storedSessionForCurrentRoute()?.credential.roomCode !== session.credential.roomCode) throw new Error("게임 연결이 변경되었습니다.");
    if (!ack.ok) {
      void requestLatestSnapshot();
      throw new ArkNovaCommandRejected(getUserErrorMessage(ack.error.code));
    }
    applyWireSnapshot(ack.data.snapshot, session);
  }
  ...
  async function actGreatKingdom(command: GreatKingdomClientCommand): Promise<void> {
    const client = clientRef.current, session = storedSessionForCurrentRoute();
    if (!client?.connected || session === null || sessionReplacedRef.current || compatibleSnapshotRef.current?.kind !== "PLATFORM_V2_GREAT_KINGDOM") throw new Error("연결을 확인하고 다시 시도해주세요.");
    const ack = await client.actGreatKingdom(command);
    if (clientRef.current !== client || sessionReplacedRef.current || storedSessionForCurrentRoute()?.playerId !== session.playerId ||
      storedSessionForCurrentRoute()?.credential.roomCode !== session.credential.roomCode) throw new Error("게임 연결이 변경되었습니다.");
    if (!ack.ok) {
      void requestLatestSnapshot();
      throw new GreatKingdomCommandRejected(getUserErrorMessage(ack.error.code));
    }
    applyWireSnapshot(ack.data.snapshot, session);
  }
  ...
  async function actPandemic(command: PandemicClientCommand): Promise<void> {
    const client = clientRef.current, session = storedSessionForCurrentRoute();
    if (!client?.connected || session === null || sessionReplacedRef.current || compatibleSnapshotRef.current?.kind !== "PLATFORM_V2_PANDEMIC") throw new Error("연결을 확인하고 다시 시도해주세요.");
    const ack = await client.actPandemic(command);
    if (clientRef.current !== client || sessionReplacedRef.current || storedSessionForCurrentRoute()?.playerId !== session.playerId ||
      storedSessionForCurrentRoute()?.credential.roomCode !== session.credential.roomCode) throw new Error("게임 연결이 변경되었습니다.");
    if (!ack.ok) {
      void requestLatestSnapshot();
      throw new PandemicCommandRejected(getUserErrorMessage(ack.error.code));
    }
    applyWireSnapshot(ack.data.snapshot, session);
  }

  async function actPerch(command: PerchClientCommand): Promise<void> {
    const client = clientRef.current, session = storedSessionForCurrentRoute();
    if (!client?.connected || session === null || sessionReplacedRef.current || compatibleSnapshotRef.current?.kind !== "PLATFORM_V2_PERCH") throw new Error("연결을 확인하고 다시 시도해주세요.");
    const ack = await client.actPerch(command);
    if (clientRef.current !== client || sessionReplacedRef.current || storedSessionForCurrentRoute()?.playerId !== session.playerId ||
      storedSessionForCurrentRoute()?.credential.roomCode !== session.credential.roomCode) throw new Error("게임 연결이 변경되었습니다.");
    if (!ack.ok) {
      void requestLatestSnapshot();
      throw new PerchCommandRejected(getUserErrorMessage(ack.error.code));
    }
    applyWireSnapshot(ack.data.snapshot, session);
  }
  ...
  async function actPatchwork(command: PatchworkClientCommand): Promise<void> {
    const client = clientRef.current, session = storedSessionForCurrentRoute();
    if (!client?.connected || session === null || sessionReplacedRef.current || compatibleSnapshotRef.current?.kind !== "PLATFORM_V2_PATCHWORK") throw new Error("연결을 확인하고 다시 시도해주세요.");
    const ack = await client.actPatchwork(command);
    if (clientRef.current !== client || sessionReplacedRef.current || storedSessionForCurrentRoute()?.playerId !== session.playerId ||
      storedSessionForCurrentRoute()?.credential.roomCode !== session.credential.roomCode) throw new Error("게임 연결이 변경되었습니다.");
    if (!ack.ok) {
      void requestLatestSnapshot();
      throw new PatchworkCommandRejected(getUserErrorMessage(ack.error.code));
    }
    applyWireSnapshot(ack.data.snapshot, session);
  }

  async function actArnak(command: ArnakClientCommand): Promise<void> {
    const client = clientRef.current, session = storedSessionForCurrentRoute();
    if (!client?.connected || session === null || sessionReplacedRef.current || compatibleSnapshotRef.current?.kind !== "PLATFORM_V2_ARNAK") throw new Error("연결을 확인하고 다시 시도해주세요.");
    const ack = await client.actArnak(command);
    if (clientRef.current !== client || sessionReplacedRef.current || storedSessionForCurrentRoute()?.playerId !== session.playerId ||
      storedSessionForCurrentRoute()?.credential.roomCode !== session.credential.roomCode) throw new Error("게임 연결이 변경되었습니다.");
    if (!ack.ok) {
      void requestLatestSnapshot();
      throw new ArnakCommandRejected(getUserErrorMessage(ack.error.code));
    }
    applyWireSnapshot(ack.data.snapshot, session);
  }
  async function actMars(command: MarsClientCommand): Promise<void> {
    const client = clientRef.current, session = storedSessionForCurrentRoute();
    if (!client?.connected || session === null || sessionReplacedRef.current || compatibleSnapshotRef.current?.kind !== "PLATFORM_V2_TERRAFORMING_MARS") throw new Error("연결을 확인하고 다시 시도해주세요.");
    const ack = await client.actMars(command);
    if (clientRef.current !== client || sessionReplacedRef.current || storedSessionForCurrentRoute()?.playerId !== session.playerId ||
      storedSessionForCurrentRoute()?.credential.roomCode !== session.credential.roomCode) throw new Error("게임 연결이 변경되었습니다.");
    if (!ack.ok) {
      void requestLatestSnapshot();
  ...
  async function actDuel(command: DuelClientCommand): Promise<void> {
    const client = clientRef.current, session = storedSessionForCurrentRoute();
    if (!client?.connected || session === null || sessionReplacedRef.current || compatibleSnapshotRef.current?.kind !== "PLATFORM_V2_SEVEN_WONDERS_DUEL") throw new Error("연결을 확인하고 다시 시도해주세요.");
    const ack = await client.actDuel(command);
    if (clientRef.current !== client || sessionReplacedRef.current || storedSessionForCurrentRoute()?.playerId !== session.playerId ||
      storedSessionForCurrentRoute()?.credential.roomCode !== session.credential.roomCode) throw new Error("게임 연결이 변경되었습니다.");
    if (!ack.ok) {
      void requestLatestSnapshot();
      throw new DuelCommandRejected(getUserErrorMessage(ack.error.code));
    }
    applyWireSnapshot(ack.data.snapshot, session);
  }
  ...
  async function actAvalon(command: AvalonClientCommand): Promise<void> {
    const client = clientRef.current, session = storedSessionForCurrentRoute();
    if (!client?.connected || session === null || sessionReplacedRef.current || compatibleSnapshotRef.current?.kind !== "PLATFORM_V2_AVALON") throw new Error("연결을 확인하고 다시 시도해주세요.");
    const ack = await client.actAvalon(command);
    if (clientRef.current !== client || sessionReplacedRef.current || storedSessionForCurrentRoute()?.playerId !== session.playerId ||
      storedSessionForCurrentRoute()?.credential.roomCode !== session.credential.roomCode) throw new Error("게임 연결이 변경되었습니다.");
    if (!ack.ok) {
      void requestLatestSnapshot();
