...
test('Branch & Claw development selection exposes ten spirits and clearly states missing content',()=>{
 ...
 assert.equal((html.match(/이 정령 선택/g)??[]).length,10);assert.match(html,/si-art-FANGS/);assert.match(html,/si-art-KEEPER/);assert.match(html,/이벤트 19\/25장 개발 덱 · 나머지 확장 카드 미포함/);
});
...
test('Event panel exposes illustrated ordered effects, preview limits and team contribution progress',()=>{
 ...
 const html=renderToStaticMarkup(createElement(SpiritScreen,{...handlers,snapshot:s}));for(const label of ['새로운 종의 확산','si-event-art','19 / 25장','토큰 이벤트','다한 이벤트','확정 전에는 소모되지 않습니다','이벤트 비용 충족도','1 더 필요'])assert.ok(html.includes(label),label);
});
