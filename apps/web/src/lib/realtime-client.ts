...
export class RealtimeClient {
  ...
  actTerrorscape(command: TerrorscapeClientCommand): Promise<StateSyncWireAck> {
    if (!parseRematch(TerrorscapeClientCommandSchema, command).success) return Promise.reject(new RealtimeClientError("INVALID_COMMAND"));
    return this.#emitAcknowledged(command.kind, command.requestId, acknowledge => {
      switch (command.kind) {
        case "terrorscape:act": this.#socket.emit("terrorscape:act", command, acknowledge); break;
      }
    }, validateStateSyncWireAck, ack => hasConsistentSnapshotAcknowledgement(ack) && this.#acceptAcknowledgementSnapshotVersion(ack));
  }

  actPandemic(command: PandemicClientCommand): Promise<StateSyncWireAck> {
    if (!parseRematch(PandemicClientCommandSchema, command).success) return Promise.reject(new RealtimeClientError("INVALID_COMMAND"));
    return this.#emitAcknowledged(command.kind, command.requestId, acknowledge => {
      switch (command.kind) {
  ...
  actHarmonies(command: HarmoniesClientCommand): Promise<StateSyncWireAck> {
    if (!parseRematch(HarmoniesClientCommandSchema, command).success) return Promise.reject(new RealtimeClientError("INVALID_COMMAND"));
    return this.#emitAcknowledged(command.kind, command.requestId, acknowledge => {
      switch (command.kind) {
  ...
  actPatchwork(command: PatchworkClientCommand): Promise<StateSyncWireAck> {
