...
import type { ArkNovaWebSnapshot } from "./snapshot-wire-decoder.js";
...
import type { PerchWebSnapshot } from "./snapshot-wire-decoder.js";
...
export type RoomSnapshotView =
  ...
  | Readonly<{ kind: "ARK_NOVA"; snapshot: ArkNovaWebSnapshot }>
  ...
  | Readonly<{ kind: "PERCH"; snapshot: PerchWebSnapshot }>
...
export function resolveRoomSnapshotView(
  ...
  if (decoded.kind === "PLATFORM_V2_ARK_NOVA") return { kind: "ARK_NOVA", snapshot: decoded.platformSnapshot };
  ...
  if (decoded.kind === "PLATFORM_V2_PERCH") return { kind: "PERCH", snapshot: decoded.platformSnapshot };
