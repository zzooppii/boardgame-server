...
import type { ArkNovaLobbyPlatformSnapshotV2, ArkNovaPlayingPlatformSnapshotV2, ArkNovaFinishedPlatformSnapshotV2 } from "@hangul-rummikub/shared";
...
import type { GreatKingdomLobbyPlatformSnapshotV2, GreatKingdomPlayingPlatformSnapshotV2, GreatKingdomFinishedPlatformSnapshotV2 } from "@hangul-rummikub/shared";
...
import type { PerchLobbyPlatformSnapshotV2, PerchPlayingPlatformSnapshotV2, PerchFinishedPlatformSnapshotV2 } from "@hangul-rummikub/shared";
...
import type { AvalonLobbyPlatformSnapshotV2, AvalonPlayingPlatformSnapshotV2, AvalonFinishedPlatformSnapshotV2 } from "@hangul-rummikub/shared";
...
export type ArkNovaWebSnapshot = ArkNovaLobbyPlatformSnapshotV2 | ArkNovaPlayingPlatformSnapshotV2 | ArkNovaFinishedPlatformSnapshotV2;
...
export type GreatKingdomWebSnapshot = GreatKingdomLobbyPlatformSnapshotV2 | GreatKingdomPlayingPlatformSnapshotV2 | GreatKingdomFinishedPlatformSnapshotV2;
...
export type PerchWebSnapshot = PerchLobbyPlatformSnapshotV2 | PerchPlayingPlatformSnapshotV2 | PerchFinishedPlatformSnapshotV2;
...
export type AvalonWebSnapshot = AvalonLobbyPlatformSnapshotV2 | AvalonPlayingPlatformSnapshotV2 | AvalonFinishedPlatformSnapshotV2;
...
export type CompatibleWebSnapshot =
  ...
  | Readonly<{kind: "PLATFORM_V2_ARK_NOVA"; snapshotVersion: 2; gameType: "ARK_NOVA"; platformSnapshot: ArkNovaWebSnapshot }>
  ...
  | Readonly<{kind: "PLATFORM_V2_GREAT_KINGDOM"; snapshotVersion: 2; gameType: "GREAT_KINGDOM"; platformSnapshot: GreatKingdomWebSnapshot }>
  ...
  | Readonly<{kind: "PLATFORM_V2_PERCH"; snapshotVersion: 2; gameType: "PERCH"; platformSnapshot: PerchWebSnapshot }>
  ...
  | Readonly<{kind: "PLATFORM_V2_AVALON"; snapshotVersion: 2; gameType: "AVALON"; platformSnapshot: AvalonWebSnapshot }>
...
function decodePlatformSnapshotV2(
  ...
  if (input.room.gameType === "ARK_NOVA") {
    const snapshot = validation.value;
    if (!isArkNovaSnapshot(snapshot)) return { kind: "INCOMPATIBLE", reason: "INVALID_V2_PROJECTION" };
    return { kind: "COMPATIBLE", value: { kind: "PLATFORM_V2_ARK_NOVA", snapshotVersion: 2, gameType: "ARK_NOVA", platformSnapshot: snapshot } };
  }
  ...
  if (input.room.gameType === "GREAT_KINGDOM") {
    const snapshot = validation.value;
    if (!isGreatKingdomSnapshot(snapshot)) return { kind: "INCOMPATIBLE", reason: "INVALID_V2_PROJECTION" };
    return { kind: "COMPATIBLE", value: { kind: "PLATFORM_V2_GREAT_KINGDOM", snapshotVersion: 2, gameType: "GREAT_KINGDOM", platformSnapshot: snapshot } };
  }
  ...
  if (input.room.gameType === "PERCH") {
    const snapshot = validation.value;
    if (!isPerchSnapshot(snapshot)) return { kind: "INCOMPATIBLE", reason: "INVALID_V2_PROJECTION" };
    return { kind: "COMPATIBLE", value: { kind: "PLATFORM_V2_PERCH", snapshotVersion: 2, gameType: "PERCH", platformSnapshot: snapshot } };
  }
  if (input.room.gameType === "HARMONIES") {
    const snapshot = validation.value;
    if (!isHarmoniesSnapshot(snapshot)) return { kind: "INCOMPATIBLE", reason: "INVALID_V2_PROJECTION" };
    return { kind: "COMPATIBLE", value: { kind: "PLATFORM_V2_HARMONIES", snapshotVersion: 2, gameType: "HARMONIES", platformSnapshot: snapshot } };
  }
  if (input.room.gameType === "PATCHWORK") {
    const snapshot = validation.value;
    if (!isPatchworkSnapshot(snapshot)) return { kind: "INCOMPATIBLE", reason: "INVALID_V2_PROJECTION" };
    return { kind: "COMPATIBLE", value: { kind: "PLATFORM_V2_PATCHWORK", snapshotVersion: 2, gameType: "PATCHWORK", platformSnapshot: snapshot } };
  }

  if (input.room.gameType === "ARNAK") {
    const snapshot = validation.value;
  ...
  if (input.room.gameType === "SEVEN_WONDERS_DUEL") {
    const snapshot = validation.value;
    if (!isDuelSnapshot(snapshot)) return { kind: "INCOMPATIBLE", reason: "INVALID_V2_PROJECTION" };
    return { kind: "COMPATIBLE", value: { kind: "PLATFORM_V2_SEVEN_WONDERS_DUEL", snapshotVersion: 2, gameType: "SEVEN_WONDERS_DUEL", platformSnapshot: snapshot } };
  }
  ...
  if (input.room.gameType === "AVALON") {
    const snapshot = validation.value;
    if (!isAvalonSnapshot(snapshot)) return { kind: "INCOMPATIBLE", reason: "INVALID_V2_PROJECTION" };
    return { kind: "COMPATIBLE", value: { kind: "PLATFORM_V2_AVALON", snapshotVersion: 2, gameType: "AVALON", platformSnapshot: snapshot } };
  }
...
function isArkNovaSnapshot(s: PlatformSnapshotV2): s is ArkNovaWebSnapshot { return s.room.gameType === "ARK_NOVA" && (s.game === null || s.game.gameType === "ARK_NOVA"); }
...
function isGreatKingdomSnapshot(s: PlatformSnapshotV2): s is GreatKingdomWebSnapshot { return s.room.gameType === "GREAT_KINGDOM" && (s.game === null || s.game.gameType === "GREAT_KINGDOM"); }
...
function isPerchSnapshot(s: PlatformSnapshotV2): s is PerchWebSnapshot { return s.room.gameType === "PERCH" && (s.game === null || s.game.gameType === "PERCH"); }
...
function isAvalonSnapshot(s: PlatformSnapshotV2): s is AvalonWebSnapshot { return s.room.gameType === "AVALON" && (s.game === null || s.game.gameType === "AVALON"); }
