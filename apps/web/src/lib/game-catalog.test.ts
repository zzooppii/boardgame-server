...
test("Web game catalog는 등록된 서른 게임을 같은 계층으로 공개한다", () => {
  assert.equal(Object.isFrozen(GAME_CATALOG), true);
  assert.equal(GAME_CATALOG.length, 30);
...
test("Home renders exactly thirty selectable game choices including the approved CITY title and capacity", () => {
  ...
  assert.equal((html.match(/class="game-option(?: selected)?"/gu) ?? []).length, 30);
