...
test('HARMONIES sounds: first view, reconnect, duplicate, old revision are silent; new committed state emits one cue',()=>{
 ...
 assert.equal(harmoniesNewCue({gameId:'g',revision:0},g,false),null);assert.equal(harmoniesNewCue({gameId:'old',revision:0},g,true),null);assert.equal(harmoniesNewCue({gameId:'g',revision:2},g,true),null);assert.equal(harmoniesNewCue({gameId:'g',revision:0},g,true),'COMMIT');
});
