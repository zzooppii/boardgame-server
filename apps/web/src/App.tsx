...
import "./features/great-kingdom/great-kingdom.css";
...
import "./features/arnak/arnak.css";
...
export function App() {
  ...
  if (
    ...
    if (roomView.kind === "GREAT_KINGDOM") {
      return <ReconnectBoundary {...recovery}><GreatKingdomScreen snapshot={roomView.snapshot}
        connected={app.connectionState === "CONNECTED" && !app.sessionReplaced && !app.resumePending && !app.reconnectNeeded} onCommand={app.actGreatKingdom} onRematch={() => app.selectRoomGame("GREAT_KINGDOM")}
        onStart={app.startGame} onLeave={app.leaveRoom} onCopy={() => app.copyInvitation(invitationUrl)}
        pending={app.operationLabel !== null || app.roomLeavePending || app.gameStartPending}
        error={app.errorMessage} connectionLabel={connectionLabel}/></ReconnectBoundary>;
    }
    ...
    if (roomView.kind === "PERCH") {
      return <ReconnectBoundary {...recovery}><PerchScreen snapshot={roomView.snapshot}
        connected={app.connectionState === "CONNECTED" && !app.sessionReplaced && !app.resumePending && !app.reconnectNeeded} onCommand={app.actPerch} onRematch={() => app.selectRoomGame("PERCH")}
        onStart={app.startGame} onLeave={app.leaveRoom} onCopy={() => app.copyInvitation(invitationUrl)}
        pending={app.operationLabel !== null || app.roomLeavePending || app.gameStartPending}
        error={app.errorMessage} connectionLabel={connectionLabel}/></ReconnectBoundary>;
    }
    if (roomView.kind === "HARMONIES") {
      return <ReconnectBoundary {...recovery}><HarmoniesScreen snapshot={roomView.snapshot}
        connected={app.connectionState === "CONNECTED" && !app.sessionReplaced && !app.resumePending && !app.reconnectNeeded} onCommand={app.actHarmonies} onRematch={() => app.selectRoomGame("HARMONIES")}
        onStart={app.startGame} onLeave={app.leaveRoom} onCopy={() => app.copyInvitation(invitationUrl)}
        pending={app.operationLabel !== null || app.roomLeavePending || app.gameStartPending}
        error={app.errorMessage} connectionLabel={connectionLabel}/></ReconnectBoundary>;
    }
    ...
    if (roomView.kind === "ARNAK") {
      return <ReconnectBoundary {...recovery}><ArnakScreen snapshot={roomView.snapshot}
        connected={app.connectionState === "CONNECTED" && !app.sessionReplaced && !app.resumePending && !app.reconnectNeeded} onCommand={app.actArnak} onRematch={() => app.selectRoomGame("ARNAK")}
        onStart={app.startGame} onLeave={app.leaveRoom} onCopy={() => app.copyInvitation(invitationUrl)}
        pending={app.operationLabel !== null || app.roomLeavePending || app.gameStartPending}
        error={app.errorMessage} connectionLabel={connectionLabel}/></ReconnectBoundary>;
    }
