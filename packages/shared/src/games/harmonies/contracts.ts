...
import { harmoniesPlayerValid } from './rules.js';
...
const Base = {gameType:v.literal('HARMONIES'),gameId:GameIdSchema,gameRevision:GameRevisionSchema,rulesVersion:v.literal('harmonies-base-a-v1'),playerStates:v.pipe(v.array(HarmoniesPlayerSchema),v.minLength(2),v.maxLength(4)),activePlayerId:PlayerIdSchema,round:v.pipe(HarmoniesCountSchema,v.minValue(1)),markets:v.pipe(v.array(v.pipe(v.array(HarmoniesTokenSchema),v.maxLength(3))),v.length(5)),animalMarket:v.pipe(v.array(HarmoniesCardIdSchema),v.maxLength(5)),bagCount:v.pipe(HarmoniesCountSchema,v.maxValue(120)),deckCount:v.pipe(HarmoniesCountSchema,v.maxValue(32)),lastRound:v.boolean(),history:v.pipe(v.array(HarmoniesLogSchema),v.maxLength(100)),privateState:v.strictObject({playerId:PlayerIdSchema})};
