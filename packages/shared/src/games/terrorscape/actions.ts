...
export const TerrorscapeActionSchema=v.variant('type',[
 ...
 v.strictObject({type:v.literal('SELECT_KILLER'),killer:TerrorKillerSchema}),
 v.strictObject({type:v.literal('SELECT_SURVIVOR'),slot:v.pipe(v.number(),v.integer(),v.minValue(0),v.maxValue(2)),character:TerrorCharacterSchema}),
 v.strictObject({type:v.literal('SETUP_READY'),ready:v.boolean()}),
