...
export const SpeakeasyLocationActionKindSchema=v.picklist(['BOOK','GOONS','PROTECT','OPERATION','BUILD','PRODUCE','DELIVER','SELL','LEVEL','FAMILY','AMBUSH','HELPER','PAID_LEVEL','CITY_TILE']);
export const SpeakeasyLocationChoiceSchema=v.variant('kind',[
  ...
  v.strictObject({kind:v.literal('DELIVER'),steps:v.pipe(v.array(v.variant('kind',[
    ...
    v.strictObject({kind:v.literal('BUY'),truckId:TileIdSchema,shipId:TileIdSchema,count:v.pipe(SpeakeasyCountSchema,v.minValue(1),v.maxValue(2)),cashToSpend:v.optional(SpeakeasyCountSchema)}),
