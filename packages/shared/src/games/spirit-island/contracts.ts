...
export const SpiritLandSchema = v.strictObject({ strifeHealthLoss:v.optional(count,0), eventBuildingHealthLoss:v.optional(v.boolean(),false), eventHealthBonus:v.optional(v.nullable(v.picklist(['BUILDINGS','EXPLORERS'])),null), eventHealthLoss:v.optional(v.boolean(),false), id: ref, board: v.picklist(['A', 'B', 'C', 'D']), number: v.pipe(count, v.minValue(0), v.maxValue(8)), terrain: SpiritTerrainSchema, coastal: v.boolean(), adjacent: v.pipe(v.array(ref), v.maxLength(12)), pieces: v.pipe(v.array(SpiritPieceSchema), v.maxLength(250)), presence: v.pipe(v.array(v.strictObject({ playerId: PlayerIdSchema, count })), v.maxLength(4)), tokens:v.optional(v.strictObject({beasts:count,wilds:count,disease:count}),{beasts:0,wilds:0,disease:0}), invaderHealth:v.optional(count,0), blight: count, defend: count, skip: v.boolean(), ravageSkip:v.optional(v.boolean(),false), protectDahan: v.boolean(), vitality: v.boolean(), dahanHealth: count });
