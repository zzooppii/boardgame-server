...
export const SpiritActionSchema = v.variant('kind', [
    ...
    v.strictObject({kind:v.literal('PLAY_LEGACY'),cardId:SpiritRefSchema}),
    v.strictObject({kind:v.literal('FLAME_RITUAL'),landId:SpiritRefSchema,cardId:SpiritRefSchema}),
    v.strictObject({kind:v.literal('USE_RELIC'),number:v.picklist([1,2,3,4,5,6,7,8])}),
    v.strictObject({kind:v.literal('SHIFT_SANDS'),from:SpiritRefSchema,to:SpiritRefSchema}),
    ...
    v.strictObject({ kind: v.literal('PLAY_CARDS'), wardCardId:v.optional(v.nullable(SpiritRefSchema)), cardIds: v.pipe(v.array(SpiritRefSchema), v.maxLength(10)) }),
