...
test('Mars desktop/mobile: hand tools, keyboard ocean placement, reconnect and sound', {timeout:120000}, async t=>{
 ...
 assert.equal(await active.locator('.tm-hex[tabindex="0"]').count(),1,'The map has one keyboard entry point.');
 const firstCell=active.locator('.tm-hex.legal').first(),lastCell=active.locator('.tm-hex.legal').last();
 const firstId=await firstCell.getAttribute('data-space-id'),lastId=await lastCell.getAttribute('data-space-id');
 await firstCell.focus();await active.keyboard.press('ArrowRight');
 assert.equal(await active.evaluate(()=>document.activeElement?.getAttribute('data-space-id')),'1-4');
 await active.keyboard.press('End');assert.equal(await active.evaluate(()=>document.activeElement?.getAttribute('data-space-id')),lastId);
 assert.equal(await active.locator('.tm-hex[tabindex="0"]').count(),1);
 assert.equal(await active.locator('.tm-hex.selected').count(),0,'Arrow navigation does not select a placement.');
 await active.keyboard.press('Tab');assert.equal(await active.evaluate(()=>document.activeElement?.classList.contains('tm-hex')),false,'Tab leaves the map in one step.');
 await lastCell.focus();await active.keyboard.press('Home');assert.equal(await active.evaluate(()=>document.activeElement?.getAttribute('data-space-id')),firstId);
 await active.keyboard.press('ArrowDown');assert.equal(await active.evaluate(()=>document.activeElement?.getAttribute('data-space-id')),'2-6');
 await active.keyboard.press('Home');await active.keyboard.press('Space');
