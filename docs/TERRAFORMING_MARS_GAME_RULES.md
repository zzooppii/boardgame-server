...
사용자의 “기본판 검증을 마무리한 뒤 기업시대 추가” 선택에 따라 기업시대 프로젝트 71장·기업 2종 추가와 시작 생산 보너스 제외를 확정한다. 게임 시작 전 방장이 선택하고 시작 후 고정한다. 근거는 기본판 규칙서 13쪽이다. 현재는 인쇄 정보·시작 자원·경제 효과 계산을 구현한 단계이며 확장 플레이는 아직 활성화하지 않는다. [구현 단계와 미완료 효과](./TERRAFORMING_MARS_CORPORATE_ERA.md)를 따른다.
