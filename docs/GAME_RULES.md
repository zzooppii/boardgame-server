...
2026-09-16 스피크이지 개발: 확인한 판정 원칙과 미확인 구성물은 [SPEAKEASY_GAME_RULES.md](./SPEAKEASY_GAME_RULES.md)에 구분한다. 현재는 경제 계산 모듈과 화면·음향 예시이며 기본판 온라인 대국은 출시하지 않았다. 기존 한글 게임의 미확정 규칙은 변경하지 않는다.

## CONFIRMED — 그레이트 킹덤 구현

사용자의 기본판 개발 요청에 따라 [그레이트 킹덤 규칙](./GREAT_KINGDOM_GAME_RULES.md)을 적용한다. 2026-09-16 사용자 정정에 따라 선후공 보정 없이 영토가 1칸이라도 많은 쪽이 승리하며 동률은 무승부로 처리한다. 기존 한글 게임 미확정 항목은 변경하지 않는다.

## CONFIRMED — 테라포밍 마스 기업시대

사용자의 기본판 검증 이후 기업시대 추가 선택에 따라 [테라포밍 마스 규칙](./TERRAFORMING_MARS_GAME_RULES.md)과 [기업시대 구현 단계](./TERRAFORMING_MARS_CORPORATE_ERA.md)를 적용한다. 기업시대는 데이터·시작 자원·경제 효과 계산 단계이며 아직 플레이 옵션으로 제공하지 않는다. 기존 한글 게임의 미확정 규칙은 변경하지 않는다.
...
사용자의 개발·일러스트·UI·상호작용·효과음 요청에 따라 [PATCHWORK_GAME_RULES.md](./PATCHWORK_GAME_RULES.md)의 2인 기본판과 방장이 선택하는 차례당 30초/60초(기본 60초), 시간 초과 시 자동 전진·가죽 자동 배치 운영을 확정한다. 기존 게임의 미확정 항목은 유지한다.
...
## CONFIRMED — 그레이트 킹덤 AI·시간 설정

2026-09-17 사용자 요청으로 초급·중급·고급 AI 및 2인 대전의 방장 선택 턴 시간(제한 없음/60/120/180/300초)을 추가한다. 세부 온라인 정책은 [그레이트 킹덤 규칙](./GREAT_KINGDOM_GAME_RULES.md)을 따른다.
